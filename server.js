...
function migrateDatabase() {
  ...
  if (db.settings.loaderTargetPackage === undefined) db.settings.loaderTargetPackage = DEFAULT_TARGET_PACKAGE;
...
function ensureUniqueKey(prefix, duration, durationLabel) {
  let key; do { key = generateLicenseKey(prefix, duration, durationLabel); } while (db.keys.some(k => k.key === key));
  return key;
}
...
function ensureOwner() {
  ...
  save();
  console.warn(`[Lexi Loader] Initial owner created: ${owner.username}. Set LEXI_OWNER_PASSWORD in production and change this password immediately.`);
}
ensureOwner();
...
app.post("/api/keys/generate", auth, (req, res) => {
  ...
  if (!db.pricing[duration] && ![ROLE.OWNER, ROLE.ADMIN].includes(req.user.role)) return res.status(400).json({ success: false, message: "Invalid duration." });
  ...
  if (db.pricing[duration]) { durationHours = Number(db.pricing[duration].hours); durationLabel = db.pricing[duration].label; basePrice = Number(db.pricing[duration].price); }
  else {
    if (![ROLE.OWNER, ROLE.ADMIN].includes(req.user.role)) return res.status(403).json({ success: false, message: "Denied." });
    durationHours = Number(req.body.customHours); durationLabel = sanitizeString(req.body.customLabel || `${durationHours} Hours`, 50); basePrice = req.user.role === ROLE.OWNER ? (customPrice === null ? 0 : customPrice) : 0;
...
app.post("/api/loader/launch", (req, res) => {
  ...
  save();
  return loaderSuccess(res, { code: "LAUNCH_AUTHORIZED", authorized: true, packageName, license: loaderLicenseSnapshot(key), loader: loaderPublicConfig(), features: loaderPublicConfig().features });
});
...
/* Backward-compatible endpoint. New Loader builds must not poll it periodically. */
