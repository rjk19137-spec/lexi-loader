 ...
 * Fully updated for Lexi Loader V9.2 Roadmap, target package com.pubg.imobile,
 * and fixed key generation duration display bug.
 */
...
/* ---------- APIs ---------- */
app.get("/api/health", (req, res) => { res.json({ success: true, service: "Lexi Loader V9.2 Backend", version: db.version, time: now() }); });
...
function loaderPublicConfig() {
  ...
  return {
    ...
    panelName: sanitizeString(s.panelName || "Lexi Loader V9.2", 100),
...
<title>Lexi Loader V9.2 Panel</title>
<style>
...
app.listen(PORT, () => {
  console.log(`Lexi Loader V9.2 backend running on port ${PORT}`);
