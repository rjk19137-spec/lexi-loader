...
async function generateKeysPage(){try{pricing=(await api('/api/pricing')).pricing||{};document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Generate Keys</h1><div class="muted">Create license keys using your available balance and permitted duration.</div></div></div><div class="grid2"><div class="card"><h3>Key Generation</h3><p class="muted">Choose duration, device limit and quantity. Key timers start after first successful loader verification.</p><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="card"><h3>Manage Keys</h3><p class="muted">View, search and manage existing licenses, device bindings and statuses.</p><button class="manage-btn" onclick="page(\'manageKeys\')">Manage Keys</button></div></div>'}catch(e){setFlash('Generate Keys load failed: '+e.message,'error');renderFlashOnly()}}
