...
function keyStatus(key) {
  ...
  if (key.expiryAt && Date.now() >= Date.parse(key.expiryAt)) return KEY_STATUS.EXPIRED;
...
function createSession(user, req, rememberMe = false) {
...
function generateLicenseKey(prefix = "LexiLoader") {
  const cleanPrefix = String(prefix || "LexiLoader")
    .replace(/[^a-zA-Z0-9]/g, "")
    .slice(0, 20) || "LexiLoader";
  const parts = [
    cleanPrefix,
...
function ensureUniqueKey(prefix) {
  let key;
  do {
...
app.post("/api/auth/register", (req, res) => {
  ...
  const user = {
    ...
    role: ROLE.RESELLER,
    status: STATUS.ACTIVE,
    parentId: creator.role === ROLE.ADMIN ? creator.id : creator.id,
  ...
  if (user.balance > 0) {
    const before = 0;
    transaction(creator, "REFERRAL_STARTING_BALANCE", user.balance, before, user.balance, user.id, {
  ...
  audit(creator, "REGISTER_USER", "user", user.id, {
    ip,
    details: { referralId: referral.id, referralCode: referral.code }
  });
  save();
...
app.post("/api/auth/reset-device", (req, res) => {
  ...
  /* Requirement: reset through username + same current IP. */
  if (user.registeredIp && user.registeredIp !== ip) {
    return res.status(403).json({
      success: false,
      message: "Device reset is allowed only from the registered IP address."
    });
  }

  resetWindow(user);
  const remainingBefore = MAX_RESET_ATTEMPTS - Number(user.resetAttempts || 0);
  if (remainingBefore <= 0) {
    save();
    return res.status(429).json({
      success: false,
      message: "Reset limit reached. Try again after the 24-hour reset window."
    });
  }

  consumeResetAttempt(user);

  const affectedKeys = db.keys.filter(k => k.creatorId === user.id && k.status !== KEY_STATUS.DELETED);
  let removedDevices = 0;

  for (const key of affectedKeys) {
    const before = db.devices.length;
    db.devices = db.devices.filter(d => d.keyId !== key.id);
    removedDevices += before - db.devices.length;
  }

  audit(user, "RESET_DEVICE_BINDINGS", "user", user.id, {
    ip,
    details: {
      affectedKeys: affectedKeys.length,
      removedDevices,
      remainingResets: MAX_RESET_ATTEMPTS - Number(user.resetAttempts || 0)
    }
  });
  ...
  res.json({
    success: true,
    message: "Device bindings reset successfully.",
    reset: {
      used: Number(user.resetAttempts),
      remaining: Math.max(0, MAX_RESET_ATTEMPTS - Number(user.resetAttempts)),
      windowStartedAt: user.resetWindowStartedAt
    }
  });
});
...
app.post("/api/auth/reset-password", (req, res) => {
  ...
  if (!user.passwordResetWindowStartedAt || Date.now() - started >= RESET_WINDOW_MS) {
    user.passwordResetAttempts = 0;
    user.passwordResetWindowStartedAt = now();
  }
  if (Number(user.passwordResetAttempts || 0) >= MAX_RESET_ATTEMPTS) return res.status(429).json({ success: false, message: "Password reset limit reached. Try again after the 24-hour window." });
...
app.get("/api/auth/reset-status", (req, res) => {
  ...
  const started = Date.parse(user.passwordResetWindowStartedAt || 0);
  if (!user.passwordResetWindowStartedAt || Date.now() - started >= RESET_WINDOW_MS) return res.json({ success: true, used: 0, remaining: MAX_RESET_ATTEMPTS });
  const used = Number(user.passwordResetAttempts || 0);
  res.json({ success: true, used, remaining: Math.max(0, MAX_RESET_ATTEMPTS - used), windowStartedAt: user.passwordResetWindowStartedAt });
});
...
app.post("/api/referrals/create", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  const code = normalizeCode(req.body.code) || normalizeCode(`LEXI-${randomToken(5)}`);
  ...
  const targetId = sanitizeString(req.body.userId || req.user.id, 80);
  ...
  if (target.role === ROLE.OWNER) {
    return res.status(403).json({ success: false, message: "Owner cannot be a referral child." });
  }
  if (!/^[A-Z0-9_-]{4,40}$/.test(code)) {
    return res.status(400).json({ success: false, message: "Invalid referral code format." });
  }
  if (db.referrals.some(r => normalizeCode(r.code) === code)) {
  ...
  audit(req.user, "CREATE_REFERRAL", "referral", referral.id, {
    ip: clientIp(req), details: { code, targetId: target.id, startingBalance, usageLimit, expiresAt }
  });
  save();
...
app.post("/api/users/:id/impersonate", auth, requireRoles(ROLE.OWNER), (req, res) => {
   ...
   */
  const token = createSession(target, req, false);
...
app.post("/api/keys/generate", auth, (req, res) => {
  ...
  if (!db.pricing[duration] && req.user.role !== ROLE.OWNER) {
  ...
  if (db.pricing[duration]) {
    ...
    if (req.user.role !== ROLE.OWNER) {
      return res.status(403).json({ success: false, message: "Custom duration is Owner-only." });
    }
    durationHours = Number(req.body.customHours);
    durationLabel = sanitizeString(req.body.customLabel || `${durationHours} Hours`, 50);
    basePrice = customPrice === null ? 0 : customPrice;
  ...
  if (req.user.role === ROLE.ADMIN && customPrice !== null) {
    return res.status(403).json({ success: false, message: "Admin cannot override fixed pricing." });
  }
  ...
  const expiryAt = durationHours
    ? new Date(createdAt + durationHours * 60 * 60 * 1000).toISOString()
    : null;
...
app.get("/api/loader/config", (req, res) => {
  res.json({
    success: true,
    loader: {
      enabled: Boolean(db.settings.loaderStatus),
...
app.post("/api/settings/update", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const allowed = [
    ...
    "resetLimit", "resetWindowHours", "ipGeoLockEnabled"
  ];
  for (const field of allowed) {
    ...
    } else if (field === "ipGeoLockEnabled") {
...
<script>
let token=localStorage.getItem('lexi_token')||'';let ownerToken=localStorage.getItem('lexi_owner_token')||'';let currentUser=null;let pricing={};let currentPage=localStorage.getItem('lexi_page')||'dashboard';let booting=false;
...
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';return '<aside id="sidebar" class="sidebar"><div class="brand">Lexi <span>Loader</span></div><div class="side-user"><b>'+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">'+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+navItem('dashboard','Dashboard')+navItem('keys','Keys')+(owner||admin?navItem('users','Users'):'')+(owner||admin?navItem('referrals','Referrals'):'')+(owner?navItem('transactions','Transactions'):'')+(owner?navItem('audit','Audit Logs'):'')+(owner?navItem('settings','Settings'):'')+'</div><div class="side-bottom">'+(ownerToken?'<button class="primary full" onclick="returnOwner()">Return to Owner</button>':'')+'<button class="action full" onclick="logout()">Logout</button></div></aside>'}
async function shell(){document.getElementById('app').innerHTML='<div class="app-shell">'+sidebar()+'<section class="main"><header class="topbar"><div class="row"><button class="action mobile-menu" onclick="document.getElementById(\'sidebar\').classList.toggle(\'open\')">Menu</button><div><b>'+esc(currentUser.role.toUpperCase())+'</b><div class="muted">'+esc(currentUser.username)+'</div></div></div><div class="row"><button class="action" onclick="toggleTheme()">Theme</button><button class="action" onclick="passwordModal()">Password</button></div></header><main class="layout" id="content"></main></section></div>'}
...
async function page(name){currentPage=name;localStorage.setItem('lexi_page',name);document.getElementById('sidebar')?.classList.remove('open');if(!currentUser)return;if(name==='dashboard')return dashboard();if(name==='keys')return keysPage();if(name==='users')return usersPage();if(name==='referrals')return referralsPage();if(name==='transactions')return transactionsPage();if(name==='audit')return auditPage();if(name==='settings'&&currentUser.role==='owner')return settingsPage();if(name==='settings')return dashboard()}
...
function generateModal(){let opts=Object.keys(pricing).map(k=>'<option value="'+esc(k)+'">'+esc(pricing[k].label)+' — ₹'+Number(pricing[k].price).toFixed(2)+'</option>').join('');let custom=currentUser.role==='owner';modal('<h2>Generate Keys</h2><div class="field"><label>Duration</label><select id="genDuration" onchange="customDurationToggle()">'+opts+(custom?'<option value="custom">Custom</option>':'')+'</select></div><div id="customFields" class="hidden"><div class="field"><label>Custom Hours</label><input id="customHours" type="number" min="1"></div><div class="field"><label>Custom Price</label><input id="customPrice" type="number" min="0" step="0.01"></div><div class="field"><label>Label</label><input id="customLabel" placeholder="Custom Duration"></div></div><div class="field"><label>Device Limit</label><input id="genDevices" type="number" min="1" max="100" value="1"></div><div class="field"><label>Key Count</label><input id="genCount" type="number" min="1" max="1000" value="1"></div><div class="field"><label>Prefix</label><input id="genPrefix" value="LexiLoader"></div><div id="genErr" class="error hidden"></div><button class="primary" onclick="generateSave()">Generate</button> <button class="action" onclick="closeModal()">Cancel</button>')}
function customDurationToggle(){document.getElementById('customFields').classList.toggle('hidden',document.getElementById('genDuration').value!=='custom')}
async function generateSave(){const er=document.getElementById('genErr');try{const duration=document.getElementById('genDuration').value;const body={duration,deviceLimit:Number(document.getElementById('genDevices').value),quantity:Number(document.getElementById('genCount').value),prefix:document.getElementById('genPrefix').value};if(duration==='custom'){body.customHours=Number(document.getElementById('customHours').value);body.customPrice=Number(document.getElementById('customPrice').value);body.customLabel=document.getElementById('customLabel').value}const d=await api('/api/keys/generate',{method:'POST',body:JSON.stringify(body)});closeModal();modal('<h2>Keys Generated</h2><p>Generated: '+d.keys.length+'</p><div class="card" style="max-height:360px;overflow:auto">'+d.keys.map(k=>'<div class="row" style="justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--line)"><span class="key">'+esc(k.key)+'</span><button class="action" onclick="copyText(\''+esc(k.key)+'\')">Copy</button></div>').join('')+'</div><button class="primary" onclick="closeModal();page(\'keys\')">Done</button>')}catch(e){er.textContent=e.message;er.classList.remove('hidden')}}
async function usersPage(){document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Users</h1><div class="muted">'+(currentUser.role==='owner'?'Owner, Admin and Reseller hierarchy':'Manage your Resellers')+'</div></div><button class="primary" onclick="createUserModal()">Create '+(currentUser.role==='owner'?'Account':'Reseller')+'</button></div><div class="card"><div class="toolbar"><input id="userSearch" placeholder="Search username / User ID"><select id="userRole"><option value="all">All Roles</option><option value="owner">Owner</option><option value="admin">Admin</option><option value="reseller">Reseller</option></select><select id="userStatus"><option value="all">All Status</option><option value="active">Active</option><option value="blocked">Blocked</option><option value="deleted">Deleted</option></select><button class="action" onclick="loadUsers()">Search</button></div><div id="usersTable"></div></div>';loadUsers()}
async function loadUsers(){try{const q=encodeURIComponent(document.getElementById('userSearch')?.value||''),role=document.getElementById('userRole')?.value||'all',status=document.getElementById('userStatus')?.value||'all';const d=await api('/api/users?search='+q+'&role='+role+'&status='+status);window.__users=d.users||[];renderUsers()}catch(e){toast(e.message)}}
function renderUsers(){const rows=window.__users||[];if(!rows.length){document.getElementById('usersTable').innerHTML='<div class="empty">No users found.</div>';return}document.getElementById('usersTable').innerHTML='<div class="table-wrap"><table class="table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Balance</th><th>Created</th><th>Actions</th></tr></thead><tbody>'+rows.map(u=>userRow(u)).join('')+'</tbody></table></div>'}
...
async function referralModal(id){try{const d=await api('/api/referrals?userId='+encodeURIComponent(id));let r=(d.referrals||[]).find(x=>x.creatorId===id);if(!r){const c=await api('/api/referrals/create',{method:'POST',body:JSON.stringify({userId:id})});r=c.referral}const link=location.origin+'/register?ref='+encodeURIComponent(r.code);modal('<h2>Referral</h2><p>Owner: <b>'+esc(id)+'</b></p><div class="field"><label>Referral Code</label><input value="'+esc(r.code)+'" readonly></div><div class="field"><label>Referral Link</label><input value="'+esc(link)+'" readonly></div><button class="primary" onclick="copyText(\''+esc(link)+'\')">Copy Link</button> <button class="action" onclick="closeModal()">Close</button>')}catch(e){toast(e.message)}}
async function referralsPage(){try{const d=await api('/api/referrals');const rows=d.referrals||[];document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Referrals</h1><div class="muted">Create and manage onboarding links</div></div><button class="primary" onclick="createReferralModal()">Create Referral</button></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Code</th><th>Creator</th><th>Used</th><th>Limit</th><th>Status</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(r=>'<tr><td><b>'+esc(r.code)+'</b></td><td>'+esc(r.creatorUsername||r.creatorId||'')+'</td><td>'+Number(r.usedCount||0)+'</td><td>'+(Number(r.usageLimit||0)>0?Number(r.usageLimit):'Unlimited')+'</td><td>'+esc(r.status)+'</td><td><button class="action" onclick="copyText(\''+esc(location.origin+'/register?ref='+r.code)+'\')">Copy Link</button><button class="action" onclick="toggleReferral(\''+esc(r.id)+'\')">'+(r.status==='active'?'Block':'Unblock')+'</button></td></tr>').join(''):'<tr><td colspan="6" class="empty">No referrals yet.</td></tr>')+'</tbody></table></div></div>'}catch(e){toast(e.message)}}
function createReferralModal(){modal('<h2>Create Referral</h2><div class="field"><label>Code (optional)</label><input id="refCode" placeholder="LEXI-ABC123"></div><div class="field"><label>Starting Balance</label><input id="refBalance" type="number" min="0" value="0"></div><div class="field"><label>Usage Limit (0 = unlimited)</label><input id="refLimit" type="number" min="0" value="0"></div><div id="refErr" class="error hidden"></div><button class="primary" onclick="saveReferral()">Create</button> <button class="action" onclick="closeModal()">Cancel</button>')}
async function saveReferral(){const er=document.getElementById('refErr');try{const d=await api('/api/referrals/create',{method:'POST',body:JSON.stringify({code:document.getElementById('refCode').value,startingBalance:Number(document.getElementById('refBalance').value||0),usageLimit:Number(document.getElementById('refLimit').value||0)})});const link=location.origin+'/register?ref='+encodeURIComponent(d.referral.code);modal('<h2>Referral Created</h2><div class="field"><input value="'+esc(link)+'" readonly></div><button class="primary" onclick="copyText(\''+esc(link)+'\')">Copy Link</button> <button class="action" onclick="closeModal();page(\'referrals\')">Done</button>')}catch(e){er.textContent=e.message;er.classList.remove('hidden')}}
...
async function settingsPage(){try{const d=await api('/api/settings'),p=await api('/api/pricing');pricing=p.pricing||{};const s=d.settings||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Settings</h1><div class="muted">Owner global controls</div></div></div><div class="settings-grid"><div class="card"><h3>Loader</h3><div class="setting-row"><div><b>Loader Status</b><small>Enable verification service</small></div><label class="switch"><input id="setLoader" type="checkbox" '+(s.loaderStatus?'checked':'')+'><span></span></label></div><div class="field"><label>Maintenance Message</label><textarea id="setMsg" rows="4">'+esc(s.maintenanceMsg||'')+'</textarea></div><button class="primary" onclick="saveLoader()">Save</button></div><div class="card"><h3>Panel</h3><div class="field"><label>Panel Name</label><input id="setName" value="'+esc(s.panelName||'')+'"></div><div class="field"><label>Support Link</label><input id="setSupport" value="'+esc(s.supportLink||'')+'"></div><div class="field"><label>Session Timeout Hours</label><input id="setSession" type="number" min="1" value="'+Number(s.sessionTimeoutHours||24)+'"></div><button class="primary" onclick="savePanel()">Save</button></div><div class="card"><h3>Security</h3><div class="setting-row"><div><b>IP / Geo Lock</b><small>Enable optional license lock fields</small></div><label class="switch"><input id="setGeo" type="checkbox" '+(s.ipGeoLockEnabled?'checked':'')+'><span></span></label></div><button class="primary" onclick="saveSecurity()">Save</button></div><div class="card"><h3>Pricing</h3><div id="priceEditor">'+Object.keys(pricing).map(k=>'<div class="pricing-row"><div><b>'+esc(pricing[k].label)+'</b><small>'+pricing[k].hours+' hours</small></div><input id="price_'+esc(k)+'" class="field-input" type="number" min="0" step="1" value="'+Number(pricing[k].price)+'" style="max-width:140px"></div>').join('')+'</div><button class="primary" onclick="savePricing()">Save Pricing</button></div></div>'}catch(e){toast(e.message)}}
