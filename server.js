...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.role !== ROLE.OWNER && user.panelDeviceId && user.panelDeviceId !== deviceId) {
    return res.status(403).json({
      success: false,
  ...
  if (user.role !== ROLE.OWNER && !user.panelDeviceId) user.panelDeviceId = deviceId;
  if (user.role !== ROLE.OWNER && !user.registeredIp) user.registeredIp = ip;
...
app.post("/api/auth/reset-password", (req, res) => {
  ...
  if (user.role !== ROLE.OWNER && user.registeredIp && user.registeredIp !== ip) return res.status(403).json({ success: false, message: "Password reset is allowed only from the registered IP address." });
...
app.get("/api/settings", auth, (req, res) => {
  ...
  res.json({ success: true, settings: { username: req.user.username, usernameChangeCount: Number(req.user.usernameChangeCount || 0), usernameChangeRemaining: req.user.role === ROLE.OWNER ? null : Math.max(0, 1 - Number(req.user.usernameChangeCount || 0)), passwordResetAttempts: Number(req.user.passwordResetAttempts || 0), passwordResetRemaining: req.user.role === ROLE.OWNER ? null : Math.max(0, MAX_RESET_ATTEMPTS - Number(req.user.passwordResetAttempts || 0)), passwordResetWindow: req.user.role === ROLE.OWNER ? "unlimited" : resetWindowLabel(req.user.role), panelDeviceResetUsed: Number(req.user.panelDeviceResetCount || 0), panelDeviceResetRemaining: req.user.role === ROLE.OWNER ? null : Math.max(0, MAX_PANEL_DEVICE_RESETS - Number(req.user.panelDeviceResetCount || 0)), theme: "local" }, role: req.user.role });
});
