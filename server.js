...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.role !== ROLE.OWNER) {
    /* Registration is tied to BOTH the panel device identity and its IP.
       Same registered device + same IP can login repeatedly. Any new
       device or IP must first complete the 3-attempt Device Reset flow. */
  ...
  /* One-panel / one-active-session rule for Admin and Reseller.
     A second login from the already-registered device is allowed, but it
     replaces the previous session so the same panel can never run in two
     places at the same time. Owner remains unrestricted. */
  if (user.role !== ROLE.OWNER) {
...
app.post("/api/referrals/:id/balance", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  const target = findUserById(referral.creatorId);
  const amount = Number(req.body.amount);
  const mode = String(req.body.mode || "set").toLowerCase();
  if (!target) return res.status(404).json({ success:false, message:"Referral creator not found." });
  ...
  else if (mode === "subtract") after = Math.max(0, before - amount);
  else if (mode !== "set") return res.status(400).json({ success:false, message:"Invalid balance mode." });
  target.balance = after;
  audit(req.user, "UPDATE_REFERRAL_CREATOR_BALANCE", "user", target.id, { ip: clientIp(req), details:{ referralId: referral.id, mode, amount, before, after } });
  save();
...
async function editReferralBalance(referralId){
  try{
    const d=await api('/api/referrals/'+encodeURIComponent(referralId));
    const r=d.referral||d;
    const ures=await api('/api/users/'+encodeURIComponent(r.creatorId));
    const u=ures.user;
    if(!u)return toast('Referral user not found.');
    modal('<h2>Edit Balance</h2><div class="field"><label>Username</label><div class="bracket">'+esc(u.username||r.creatorId)+'</div></div><div class="field"><label>Current Balance</label><div class="bracket">₹'+Number(u.balance||0).toFixed(2)+'</div></div><div class="field"><label>Amount</label><input id="refBalAmount" type="number" min="0" step="0.01" value="0"></div><div class="field"><label>Action</label><select id="refBalMode"><option value="add">Add</option><option value="subtract">Subtract</option><option value="set">Set</option></select></div><div id="refBalErr" class="error hidden"></div><div class="row" style="justify-content:flex-end;gap:8px"><button class="primary" onclick="saveReferralBalance(&quot;'+esc(r.id)+'&quot;)">Save</button><button class="action" onclick="closeModal()">Cancel</button></div>');
  }catch(e){toast(e.message)}
}
async function saveReferralBalance(referralId){
  const er=document.getElementById('refBalErr');
  try{
    const amount=Number(document.getElementById('refBalAmount')?.value);
    const mode=String(document.getElementById('refBalMode')?.value||'set');
    if(!Number.isFinite(amount)||amount<0){er.textContent='Invalid balance amount.';er.classList.remove('hidden');return;}
    await api('/api/referrals/'+encodeURIComponent(referralId)+'/balance',{method:'POST',body:JSON.stringify({mode,amount})});
    closeModal();setFlash('Balance updated successfully.','success');referralsPage();
