...
app.get("/api/loader/config", (req, res) => {
  res.json({ success: true, loader: { enabled: Boolean(db.settings.loaderStatus) && db.settings.systemOnline !== false, systemOnline: db.settings.systemOnline !== false, maintenanceMessage: db.settings.maintenanceMsg, panelName: db.settings.panelName, appNoticeEnabled: Boolean(db.settings.appNoticeEnabled), appNotice: db.settings.appNotice || "", appNoticeType: db.settings.appNoticeType || "info" }, pricing: db.pricing });
});
app.post("/api/owner/loader-settings", auth, requireRoles(ROLE.OWNER), (req, res) => {
  ...
  audit(req.user, "UPDATE_LOADER_SETTINGS", "settings", "global", { ip: clientIp(req), details: { loaderStatus: db.settings.loaderStatus, ipGeoLockEnabled: db.settings.ipGeoLockEnabled } }); save();
  res.json({ success: true, settings: { loaderStatus: db.settings.loaderStatus, maintenanceMsg: db.settings.maintenanceMsg, panelName: db.settings.panelName, ipGeoLockEnabled: db.settings.ipGeoLockEnabled } });
});
app.post("/api/loader/verify", (req, res) => {
  const licenseKey = sanitizeString(req.body.key, 150); const hwid = sanitizeString(req.body.hwid, 300); const deviceName = sanitizeString(req.body.deviceName, 150); const ip = clientIp(req);
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return res.status(503).json({ success: false, code: "LOADER_DISABLED", message: db.settings.maintenanceMsg });
  if (!licenseKey || !hwid) return res.status(400).json({ success: false, code: "INVALID_REQUEST", message: "Required." });
  const key = db.keys.find(k => k.key === licenseKey); if (!key || key.status === KEY_STATUS.DELETED) return res.status(404).json({ success: false, code: "KEY_NOT_FOUND", message: "Not found." });
  const status = keyStatus(key);
  if (status === KEY_STATUS.BLOCKED) return res.status(403).json({ success: false, code: "KEY_BLOCKED", message: "Blocked." });
  if (status === KEY_STATUS.EXPIRED) return res.status(403).json({ success: false, code: "KEY_EXPIRED", message: "Expired." });
  if (key.ipLock && key.ipLock !== ip) return res.status(403).json({ success: false, code: "IP_LOCKED", message: "IP Locked." });
  if (key.geoLock) { const country = String(req.headers["x-country-code"] || "").toUpperCase(); if (!country || country !== String(key.geoLock).toUpperCase()) return res.status(403).json({ success: false, code: "GEO_LOCKED", message: "Geo Locked." }); }
  const devices = db.devices.filter(d => d.keyId === key.id); let device = devices.find(d => safeEqual(d.hwid, hwid)); const firstActivation = !key.startedAt;
  if (!device) {
    if (devices.length >= Number(key.deviceLimit || 1)) return res.status(403).json({ success: false, code: "DEVICE_LIMIT_REACHED", message: "Limit reached." });
    ...
    device = { id: randomId("dev"), keyId: key.id, hwid, deviceName, ip, firstSeenAt: now(), lastSeenAt: now() }; db.devices.push(device);
  } else {
    if (firstActivation) { const startedMs = Date.now(); key.startedAt = new Date(startedMs).toISOString(); key.expiryAt = new Date(startedMs + Number(key.durationHours || 0) * 60 * 60 * 1000).toISOString(); }
    device.lastSeenAt = now(); device.ip = ip; if (deviceName) device.deviceName = deviceName;
  }
  audit(null, "LOADER_VERIFY", "key", key.id, { ip, details: { hwid: hwid.slice(0, 12), deviceName, devicesUsed: db.devices.filter(d => d.keyId === key.id).length } }); save();
  res.json({ success: true, code: "VERIFIED", license: { key: key.key, startedAt: key.startedAt || null, expiryAt: key.expiryAt || null, durationHours: Number(key.durationHours || 0), deviceLimit: key.deviceLimit, devicesUsed: db.devices.filter(d => d.keyId === key.id).length }, loader: { name: db.settings.panelName, enabled: true } });
});
