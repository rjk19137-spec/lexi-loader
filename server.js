...
.app-shell{min-height:100vh}.sidebar{position:fixed;left:0;top:0;bottom:0;width:var(--sidebar);background:#0f131b;border-right:1px solid var(--line);padding:20px 14px;z-index:50;display:flex;flex-direction:column}.side-user{padding:15px 12px;margin:18px 0 12px;background:var(--panel);border:1px solid var(--line);border-radius:12px}.side-nav{display:grid;gap:6px}.navbtn{width:100%;text-align:left;background:transparent;color:#cfd5df;border:1px solid transparent;border-radius:9px;padding:11px 12px}.navbtn:hover,.navbtn.active{background:#1a202c;border-color:var(--line);color:#fff}.side-bottom{margin-top:auto;display:grid;gap:7px}.main{margin-left:var(--sidebar);min-height:100vh}.topbar{height:72px;padding:0 24px;border-bottom:1px solid var(--line);background:rgba(11,14,20,.94);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:30}.mobile-menu{display:none}.layout{max-width:1450px;margin:auto;padding:24px}.page-head{display:flex;justify-content:space-between;align-items:center;gap:14px;margin-bottom:20px}.page-head h1{margin:0 0 5px;font-size:28px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:18px;box-shadow:var(--shadow)}.stat .label{color:var(--muted)}.stat b{display:block;font-size:29px;margin-top:9px}.toolbar{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:9px;margin-bottom:14px}.table-wrap{overflow:auto}.table{width:100%;border-collapse:collapse;min-width:920px}.table th,.table td{padding:12px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}.table th{color:#aeb7c7}.status-active{color:#68cf98;font-weight:700}.status-blocked{color:#e66c74;font-weight:700}.bracket{background:#fff;color:#111;border:1px solid #d7dbe2;border-radius:9px;padding:11px}.list-box{display:grid;margin-top:5px}.list-box button{padding:10px;text-align:left;border:1px solid var(--line);background:var(--panel2);color:var(--text)}.badge{display:inline-block;padding:4px 9px;border-radius:99px;background:#29303b;font-size:12px}.badge.ok{color:#a9ebc6}.badge.bad{color:#ffb7bd}.actions{display:flex;flex-wrap:wrap;gap:6px}.primary,.action,.danger{padding:10px 14px;border-radius:9px;border:1px solid transparent}.primary{background:var(--accent);color:#171717;font-weight:750}.action{background:#202633;color:var(--text);border-color:#303949}.danger{background:#46252a;color:#ffb5ba;border-color:#6c353d}.full{width:100%}.field{margin:13px 0}.field label{display:block;margin-bottom:7px;color:#ccd3df}.field-input,input,select,textarea{width:100%;background:#0d1118;color:var(--text);border:1px solid var(--line);border-radius:9px;padding:11px;outline:none}.field-input:focus,input:focus,select:focus,textarea:focus{border-color:#657084}.row{display:flex;gap:9px;align-items:center}.wrap{flex-wrap:wrap}.empty{text-align:center;padding:40px;color:var(--muted)}.key{font-family:monospace;font-size:13px}.login-wrap{min-height:100vh;display:grid;place-items:center;padding:20px}.login{width:min(430px,100%);background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:30px;box-shadow:var(--shadow)}.error{color:#ffb4ba;background:#351d21;border:1px solid #63333a;padding:10px;border-radius:8px;margin:10px 0}.flash{margin-bottom:14px;padding:12px 15px;border-radius:9px;font-weight:700;border:1px solid}.flash.success{color:#63d493;background:#102a1d;border-color:#2f744b}.flash.error{color:#ff6f79;background:#35191e;border-color:#74343b}.toast{position:fixed;right:18px;bottom:18px;background:#202733;border:1px solid var(--line);padding:12px 15px;border-radius:9px;z-index:100}.modal-bg{position:fixed;inset:0;background:rgba(0,0,0,.76);display:grid;place-items:center;padding:18px;z-index:90}.modal{width:min(650px,100%);max-height:90vh;overflow:auto;background:var(--panel);border:1px solid var(--line);border-radius:15px;padding:20px}.modal h2{margin-top:0}.setting-row,.pricing-row{display:flex;justify-content:space-between;gap:15px;align-items:center;padding:13px 0;border-bottom:1px solid var(--line)}.settings-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.switch{position:relative;width:48px;height:26px}.switch input{display:none}.switch span{position:absolute;inset:0;background:#343b49;border-radius:99px}.switch span:after{content:"";position:absolute;width:20px;height:20px;left:3px;top:3px;background:#fff;border-radius:50%;transition:.15s}.switch input:checked+span{background:#5c8b6f}.switch input:checked+span:after{transform:translateX(22px)}
...
function navItem(id,label){return '<button class="navbtn '+(currentPage===id?'active':'')+'" onclick="page(\''+id+'\')">'+label+'</button>'}
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';let items=navItem('dashboard','Dashboard')+navItem('keys','Keys');if(owner)items+=navItem('users','Users');items+=navItem('referrals','Referrals');if(owner||admin)items+=navItem('transactions','Transactions')+navItem('activity','Activity');if(owner)items+=navItem('audit','Audit Logs')+navItem('settings','Settings')+navItem('online','Online');else items+=navItem('settings','Settings');return '<aside id="sidebar" class="sidebar"><div class="brand">Lexi <span>Loader</span></div><div class="side-user"><b>'+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">'+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+items+'</div><div class="side-bottom">'+(ownerToken&&owner?'<button class="primary full" onclick="returnOwner()">Return to Owner</button>':'')+'<button class="action full" onclick="logout()">Logout</button></div></aside>'}
...
async function page(name){if(name!==currentPage)window.__generatedKeys=null;currentPage=name;localStorage.setItem('lexi_page',name);document.getElementById('sidebar')?.classList.remove('open');if(!currentUser)return;if(name==='dashboard')return dashboard();if(name==='keys')return keysPage();if(name==='users'&&currentUser.role==='owner')return usersPage();if(name==='referrals')return referralsPage();if(name==='transactions'&&(currentUser.role==='owner'||currentUser.role==='admin'))return transactionsPage();if(name==='activity'&&(currentUser.role==='owner'||currentUser.role==='admin'))return activityPage();if(name==='audit'&&currentUser.role==='owner')return auditPage();if(name==='settings')return settingsPage();if(name==='online'&&currentUser.role==='owner')return onlinePage();if(name==='settings'||name==='online')return dashboard()}
...
async function referralsPage(){try{const d=await api('/api/referrals');const rows=d.referrals||[];if(currentUser.role==='admin'){const users=(await api('/api/users')).users||[];const mine=users.filter(u=>u.role==='reseller'&&u.parentId===currentUser.id&&u.status!=='deleted');document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Referrals</h1><div class="muted">Your referred Resellers only</div></div><button class="primary" onclick="createReferralModal()">Create Referral</button></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Username</th><th>Balance</th><th>Status</th><th>Actions</th></tr></thead><tbody>'+(mine.length?mine.map(u=>'<tr><td><b>'+esc(u.username)+'</b></td><td>₹'+Number(u.balance||0).toFixed(2)+'</td><td><span class="status-text '+(u.status==='active'?'status-active':'status-blocked')+'">'+(u.status==='active'?'Active':'Block')+'</span></td><td class="actions"><button class="action" onclick="manageReseller(\''+esc(u.id)+'\')">Manage</button><button class="danger" onclick="deleteReseller(\''+esc(u.id)+'\')">Delete</button></td></tr>').join(''):'<tr><td colspan="4" class="empty">No referred Resellers yet.</td></tr>')+'</tbody></table></div></div>';return}document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Referrals</h1><div class="muted">Create and manage onboarding links</div></div><button class="primary" onclick="createReferralModal()">Create Referral</button></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Code</th><th>Creator</th><th>Used</th><th>Limit</th><th>Status</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(r=>'<tr><td><b>'+esc(r.code)+'</b></td><td>'+esc(r.creatorUsername||r.creatorId||'')+'</td><td>'+Number(r.usedCount||0)+'</td><td>'+(Number(r.usageLimit||0)>0?Number(r.usageLimit):'Unlimited')+'</td><td>'+esc(r.status)+'</td><td><button class="action" onclick="toggleReferral(\''+esc(r.id)+'\')">'+(r.status==='active'?'Block':'Unblock')+'</button></td></tr>').join(''):'<tr><td colspan="6" class="empty">No referrals yet.</td></tr>')+'</tbody></table></div></div>'}catch(e){toast(e.message)}}
