...
<script>
let token=localStorage.getItem('lexi_token')||'';let ownerToken=localStorage.getItem('lexi_owner_token')||'';let currentUser=null;let pricing={};let currentPage='dashboard';window.__panelFlash=null;window.__generatedKeys=null;let booting=false;
...
const PAGE_PATHS={dashboard:'/Dashboard',generateKeys:'/Generate-Keys',manageKeys:'/Manage-Keys',users:'/Users',referrals:'/Referrals',transactions:'/Transactions',activity:'/Activity',audit:'/Audit-Logs',settings:'/Settings',online:'/Online'};
...
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';let items=navItem('dashboard','Dashboard')+navItem('generateKeys','Generate Keys')+navItem('manageKeys','Manage Keys');if(owner)items+=navItem('users','Users');items+=navItem('referrals','Referrals');if(owner||admin)items+=navItem('transactions','Transactions')+navItem('activity','Activity');if(owner)items+=navItem('audit','Audit Logs')+navItem('settings','Settings')+navItem('online','Online');else items+=navItem('settings','Settings');return '<div id="sideOverlay" class="side-overlay" onclick="closeSideMenu()"></div><aside id="sidebar" class="sidebar"><div class="brand">Lexi <span>Loader</span></div><div class="side-user"><b>'+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">'+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+items+'</div><div class="side-bottom">'+(ownerToken&&owner?'<button class="primary full" onclick="returnOwner()">Return to Owner</button>':'')+'<button class="action full" onclick="logout()">Logout</button></div></aside>'}
...
async function page(name,opts={}){const push=opts.push!==false;const changed=name!==currentPage;if(changed){window.__generatedKeys=null;window.__panelFlash=null;const fa=document.getElementById('flashArea');if(fa)fa.innerHTML='';}currentPage=name;localStorage.setItem('lexi_page',name);if(push&&location.pathname.toLowerCase()!==pagePath(name).toLowerCase())history.pushState({page:name},'',pagePath(name));closeSideMenu();document.querySelectorAll('.navbtn').forEach(b=>b.classList.toggle('active',b.dataset.page===name));if(!currentUser)return;if(name==='dashboard')return dashboard();if(name==='generateKeys')return generateKeysPage();if(name==='manageKeys')return keysPage();if(name==='users'&&currentUser.role==='owner')return usersPage();if(name==='referrals')return referralsPage();if(name==='transactions'&&(currentUser.role==='owner'||currentUser.role==='admin'))return transactionsPage();if(name==='activity'&&(currentUser.role==='owner'||currentUser.role==='admin'))return activityPage();if(name==='audit'&&currentUser.role==='owner')return auditPage();if(name==='settings')return settingsPage();if(name==='online'&&currentUser.role==='owner')return onlinePage();return dashboard()}
...
async function generateKeysPage(){try{pricing=(await api('/api/pricing')).pricing||{};document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Generate Keys</h1><div class="muted">Create license keys using your available balance and permitted duration.</div></div><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="grid2"><div class="card"><h3>Key Generation</h3><p class="muted">Choose duration, device limit and quantity. Key timers start after first successful loader verification.</p><button class="primary" onclick="generateModal()">Open Generator</button></div><div class="card"><h3>Manage Keys</h3><p class="muted">View, search and manage existing licenses, device bindings and statuses.</p><button class="action" onclick="page(\'manageKeys\')">Open Manage Keys</button></div></div>'}catch(e){setFlash('Generate Keys load failed: '+e.message,'error');renderFlashOnly()}}
async function keysPage(){try{const d=await api('/api/keys');pricing=(await api('/api/pricing')).pricing||{};document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Manage Keys</h1><div class="muted">Manage licenses and device bindings</div></div></div><div class="card"><div class="toolbar"><input id="keySearch" placeholder="Search license / HWID"><select id="keyStatus"><option value="all">All Status</option><option value="active">Active</option><option value="blocked">Blocked</option><option value="expired">Expired</option><option value="deleted">Deleted</option></select><select id="keyDevice"><option value="all">All Devices</option><option value="bound">Bound</option><option value="unbound">Unbound</option></select><button class="action" onclick="loadKeys()">Search</button></div><div id="keysTable"></div></div>';window.__keys=d.keys||[];renderKeys()}catch(e){setFlash('Failed: '+e.message,'error');renderFlashOnly()}}
...
async function passwordModal(){modal('<h2>Change Password</h2><div class="field"><label>New Password</label><div class="password-wrap"><input id="newPassword" type="password" minlength="6"><button type="button" class="eye-btn" onclick="togglePassword(\'newPassword\',this)">Show</button></div></div><div id="pwErr" class="error hidden"></div><button class="primary" onclick="changePassword()">Save</button> <button class="action" onclick="closeModal()">Cancel</button>')}
...
app.get(/^\/(Dashboard|Generate-Keys|Manage-Keys|Users|Referrals|Transactions|Activity|Audit-Logs|Settings|Online)(?:\/.*)?$/i, (req, res) => res.type("html").send(PANEL_HTML));
