 ...
 * Fully updated for Lexi Loader V10.0 Roadmap, target package com.pubg.imobile,
 * added server-sync mode endpoint, and updated version branding.
 */
...
function initialDatabase() {
  return {
    ...
    settings: {
      loaderStatus: true,
      maintenanceMsg: "Loader is temporarily under maintenance."[cite: 3],
      panelName: "Lexi Loader V10.0",
      supportLink: "",
      ...
      resetWindowHours: 24,
      systemOnline: true,
      onlineMessage: "System is online."[cite: 3],
      appNotice: "",
      appNoticeType: "info",
...
function migrateDatabase() {
  ...
  if (db.settings.onlineMessage === undefined) db.settings.onlineMessage = "System is online."[cite: 3];
...
function loaderPublicConfig() {
  ...
  return {
    ...
    maintenanceMessage: sanitizeString(s.maintenanceMsg || "Loader is temporarily under maintenance."[cite: 3], 1000),
    panelName: sanitizeString(s.panelName || "Lexi Loader V10.0", 100),
...
function checkLoaderAuthorization(session, packageName, loaderVersion, req) {
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return { status: 503, code: "LOADER_DISABLED", message: db.settings.maintenanceMsg || "Loader is temporarily under maintenance."[cite: 3] };
...
// Added V10.0 sync-mode endpoint for PostLaunchManager server synchronization
...
app.post("/api/loader/verify", (req, res) => {
  ...
  const ip = clientIp(req);
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return loaderResponse(res, 503, "LOADER_DISABLED", db.settings.maintenanceMsg || "Loader is temporarily under maintenance."[cite: 3]);
...
<title>Lexi Loader V10.0 Panel</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
:root{--bg:#09090b;--panel:#121216;--panel2:#18181d;--line:rgba(255,255,255,0.06);--text:#ededed;--muted:#82828b;--accent:#d4af37;--danger:#e84c4c;--ok:#25a753;--sidebar:260px;--shadow:0 12px 40px rgba(0,0,0,0.6)}
...
.brand span{background:linear-gradient(135deg,#d4af37,#f3e5ab);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.muted,small{color:var(--muted)}
.app-shell{min-height:100vh}
.sidebar{position:fixed;left:0;top:0;bottom:0;width:var(--sidebar);background:rgba(9,9,11,0.95);backdrop-filter:blur(10px);border-right:1px solid var(--line);padding:24px 16px;z-index:50;display:flex;flex-direction:column}
.side-user{padding:16px;margin:20px 0 16px;background:var(--panel2);border:1px solid var(--line);border-radius:14px;box-shadow:inset 0 1px 0 rgba(255,255,255,0.02)}
.side-nav{display:grid;gap:8px}
.navbtn{width:100%;text-align:left;background:transparent;color:#a1a1aa;border:1px solid transparent;border-radius:10px;padding:12px 14px;font-weight:500}
.navbtn:hover,.navbtn.active{background:rgba(212,175,55,0.08);color:var(--accent);border-color:rgba(212,175,55,0.2)}
.side-bottom{margin-top:auto;display:grid;gap:8px}
.main{margin-left:var(--sidebar);min-height:100vh}
.topbar{height:76px;padding:0 28px;border-bottom:1px solid var(--line);background:rgba(9,9,11,0.8);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:30}
...
.page-head h1{margin:0 0 6px;font-size:32px;font-weight:700;letter-spacing:-0.5px}
.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}
.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:22px;box-shadow:var(--shadow);position:relative;overflow:hidden}
.card::before{content:"";position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(180deg,rgba(255,255,255,0.02) 0%,transparent 100%);pointer-events:none}
.stat .label{color:var(--muted);font-weight:500;text-transform:uppercase;font-size:12px;letter-spacing:0.5px}
.stat b{display:block;font-size:34px;margin-top:10px;font-weight:800;background:linear-gradient(to right,#fff,#a1a1aa);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.bracket{background:var(--panel2);color:var(--text);border:1px solid var(--line);border-radius:10px;padding:12px}
.list-box{display:grid;margin-top:6px}
.list-box button{padding:12px;text-align:left;border:1px solid var(--line);background:var(--panel2);color:var(--text)}
.badge{display:inline-block;padding:5px 12px;border-radius:99px;background:#1f1f22;font-size:12px;font-weight:600;border:1px solid rgba(255,255,255,0.05)}
.badge.ok{color:#4ade80;background:rgba(74,222,128,0.1);border-color:rgba(74,222,128,0.2)}
.badge.bad{color:#f87171;background:rgba(248,113,113,0.1);border-color:rgba(248,113,113,0.2)}
.actions{display:flex;flex-wrap:wrap;gap:8px}
.primary,.action,.danger{padding:11px 16px;border-radius:10px;border:1px solid transparent;font-weight:600}
.primary{background:linear-gradient(135deg,#d4af37,#b5901e);color:#000;box-shadow:0 4px 14px rgba(212,175,55,0.2)}
.primary:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(212,175,55,0.3)}
.golden-btn{background:linear-gradient(135deg,#d4af37,#b5901e)!important;color:#000!important;border:none!important}
.generate-btn{background:linear-gradient(135deg,#d4af37,#b5901e)!important;color:#000!important;border:none!important;font-weight:700;box-shadow:0 4px 14px rgba(212,175,55,0.3)!important}
.manage-btn{background:rgba(212,175,55,0.1)!important;color:var(--accent)!important;border-color:rgba(212,175,55,0.2)!important}
.manage-btn:hover{background:rgba(212,175,55,0.2)!important}
.modal-actions{display:inline-flex;align-items:center;gap:10px;flex-wrap:wrap}
.action{background:var(--panel2);color:var(--text);border-color:var(--line)}
.action:hover{background:#27272f}
.danger{background:rgba(232,76,76,0.1);color:#f87171;border-color:rgba(232,76,76,0.2)}
.danger:hover{background:rgba(232,76,76,0.2)}
.block-btn{background:rgba(232,76,76,0.1)!important;color:#f87171!important;border-color:rgba(232,76,76,0.2)!important}
.unblock-btn{background:rgba(74,222,128,0.1)!important;color:#4ade80!important;border-color:rgba(74,222,128,0.2)!important}
.full{width:100%}
.settings-actions{gap:16px;margin-top:2px;align-items:center}
.owner-security-actions{display:flex;align-items:center;gap:16px;flex-wrap:wrap}
.username-limit-row{margin-top:10px;padding-left:4px;justify-content:flex-start;gap:12px;flex-wrap:wrap}
.username-save-btn{margin-left:4px}
.field{margin:16px 0}
.field label{display:block;margin-bottom:8px;color:#a1a1aa;font-weight:500}
.field-input,input,select,textarea{width:100%;background:#09090b;color:var(--text);border:1px solid var(--line);border-radius:10px;padding:13px 14px;outline:none;transition:all 0.2s;font-family:inherit}
.field-input:focus,input:focus,select:focus,textarea:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(212,175,55,0.15)}
.row{display:flex;gap:10px;align-items:center}
.wrap{flex-wrap:wrap}
.empty{text-align:center;padding:50px;color:var(--muted);font-style:italic}
.key{font-family:'Courier New',Courier,monospace;font-size:14px;background:rgba(255,255,255,0.05);padding:4px 8px;border-radius:6px}
.login-wrap{min-height:100vh;display:grid;place-items:center;padding:20px;background:radial-gradient(circle at top, #1a1a24 0%, #09090b 100%)}
.login{width:min(440px,100%);background:rgba(18,18,22,0.6);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.1);border-radius:20px;padding:36px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.8)}
.error{color:#f87171;background:rgba(248,113,113,0.1);border:1px solid rgba(248,113,113,0.2);padding:12px;border-radius:10px;margin:12px 0;font-weight:500}
...
.flash.success{color:#4ade80;background:rgba(74,222,128,0.1);border-color:rgba(74,222,128,0.2)}
.flash.error{color:#f87171;background:rgba(248,113,113,0.1);border-color:rgba(248,113,113,0.2)}
.toast{position:fixed;right:24px;bottom:24px;background:var(--panel2);border:1px solid var(--line);padding:14px 20px;border-radius:12px;z-index:100;box-shadow:0 10px 30px rgba(0,0,0,0.5);font-weight:500}
.modal-bg{position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(5px);display:grid;place-items:center;padding:20px;z-index:90}
.modal{width:min(650px,100%);max-height:90vh;overflow:auto;background:var(--panel);border:1px solid var(--line);border-radius:20px;padding:28px;box-shadow:0 25px 50px rgba(0,0,0,0.6)}
.modal h2{margin-top:0;font-size:24px;margin-bottom:16px}
...
.session-choice{display:flex;gap:12px;margin-top:10px}
.session-option{flex:1;position:relative}
.session-option input{position:absolute;opacity:0;pointer-events:none}
.session-option label{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border:1px solid var(--line);border-radius:12px;background:var(--panel2);cursor:pointer;color:var(--text);font-weight:500;transition:all 0.2s}
.session-option input:checked+label{border-color:var(--accent);background:rgba(212,175,55,0.05);box-shadow:0 0 0 1px var(--accent)}
.session-option label:after{content:"✓";font-weight:800;color:var(--accent);opacity:0;transition:all 0.2s}
.session-option input:checked+label:after{opacity:1}
.setting-row,.pricing-row{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:16px 0;border-bottom:1px solid var(--line)}
.settings-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}
.switch{position:relative;width:52px;height:28px}
.switch input{display:none}
.switch span{position:absolute;inset:0;background:#27272a;border-radius:99px;transition:0.3s;border:1px solid var(--line)}
...
@media(max-width:900px){:root{--sidebar:0px}.sidebar{display:none}.sidebar.open{display:flex;width:280px;box-shadow:20px 0 60px rgba(0,0,0,0.8)}.side-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(3px);z-index:40}.side-overlay.open{display:block}.main{margin-left:0}.mobile-menu{display:inline-flex;padding:8px}.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.settings-grid,.grid2{grid-template-columns:1fr}.toolbar{grid-template-columns:1fr 1fr}.toolbar input{grid-column:1/-1}.topbar{padding:0 20px}}
@media(max-width:550px){.layout{padding:16px}.grid{grid-template-columns:1fr}.toolbar{grid-template-columns:1fr}.page-head{align-items:flex-start;flex-direction:column}.topbar{height:68px}.page-head h1{font-size:26px}.actions .action,.actions .danger{padding:10px 14px}}
.session-timeout-heading{font-size:14px}.session-timeout-options .session-option label{font-size:12px!important;padding:12px 14px;justify-content:flex-start;gap:10px}.session-timeout-options .session-option label:after{font-size:12px;order:-1}
</style>
</head>
<body>
...
app.listen(PORT, () => {
  console.log(`Lexi Loader V10.0 backend running on port ${PORT}`);
