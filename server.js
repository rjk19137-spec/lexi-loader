...
function getSession(req) {
  ...
  /* Sliding inactivity window: every request extends the selected session timeout. */
  const last = Date.parse(session.lastActivityAt);
  const configuredHours = Number(session.timeoutHours || db.settings?.sessionTimeoutHours || 24);
  const timeoutHours = ALLOWED_SESSION_HOURS.includes(configuredHours) ? configuredHours : 24;
  const inactivityMs = timeoutHours * 60 * 60 * 1000;
...
app.post("/api/auth/login", (req, res) => {
  ...
  const sessionMaxAge = rememberMe ? 30 * 24 * 60 * 60 * 1000 : sessionTimeoutHours * 60 * 60 * 1000;
  ...
  res.json({
    ...
    session: {
      timeoutHours: rememberMe ? 720 : sessionTimeoutHours,
      rememberMe
    }
  });
});
