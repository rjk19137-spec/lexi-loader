...
function createSession(user, req, rememberMe = false, supportSession = false) {
  const token = randomToken(32);
  const created = Date.now();
  const maxAge = rememberMe ? 30 * 24 * 60 * 60 * 1000 : SESSION_TTL_MS;
...
function getSession(req) {
  ...
  /* Sliding inactivity window: every request extends the 24h timeout. */
  ...
  const newLast = now();
  session.lastActivityAt = newLast;
...
function auth(req, res, next) {
  ...
  if (!result.session.supportSession) {
    const deviceId = sanitizeString(req.headers["x-panel-device-id"] || "", 160);
    if (!deviceId || !req.user.panelDeviceId || deviceId !== req.user.panelDeviceId) {
      return res.status(403).json({ success: false, code: "PANEL_DEVICE_MISMATCH", message: "This account is authorized on another panel device. Reset the panel device to continue.", resetRemaining: Math.max(0, MAX_PANEL_DEVICE_RESETS - Number(req.user.panelDeviceResetCount || 0)) });
    }
  }
  next();
}
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (!deviceId) {
    return res.status(400).json({ success: false, code: "PANEL_DEVICE_REQUIRED", message: "Panel device identifier is required." });
  }
  if (user.panelDeviceId && user.panelDeviceId !== deviceId) {
    return res.status(403).json({
      success: false,
      code: "PANEL_DEVICE_MISMATCH",
      message: "This account is already active on another device. Reset the panel device to continue.",
      resetRemaining: Math.max(0, MAX_PANEL_DEVICE_RESETS - Number(user.panelDeviceResetCount || 0))
    });
  }
  if (!user.panelDeviceId) user.panelDeviceId = deviceId;
  if (!user.registeredIp) user.registeredIp = ip;
  clearFailedLogin(ip, username);

  user.lastLoginAt = now();
  const token = createSession(user, req, rememberMe);
  setSessionCookie(res, token, rememberMe ? 30 * 24 * 60 * 60 * 1000 : SESSION_TTL_MS);
  ...
  res.json({
    ...
    session: {
      timeoutHours: rememberMe ? 720 : 24,
      rememberMe
    }
  });
});
...
app.post("/api/auth/register", (req, res) => {
  ...
  if (Number(referral.usageLimit || 0) > 0 && Number(referral.usedCount || 0) >= Number(referral.usageLimit)) {
    return res.status(400).json({ success: false, message: "Referral usage limit reached." });
  }
...
app.post("/api/auth/reset-device", (req, res) => {
  ...
  if (Number(user.panelDeviceResetCount || 0) >= MAX_PANEL_DEVICE_RESETS) {
...
app.post("/api/auth/reset-password", (req, res) => {
  ...
  if (user.registeredIp && user.registeredIp !== ip) return res.status(403).json({ success: false, message: "Password reset is allowed only from the registered IP address." });
  ...
  if (Number(user.passwordResetAttempts || 0) >= MAX_RESET_ATTEMPTS) return res.status(429).json({ success: false, message: "Password reset limit reached. Try again after the " + resetWindowLabel(user.role) + " window." });
...
app.post("/api/keys/generate", auth, (req, res) => {
  ...
  const prefix = sanitizeString(req.body.prefix || "LexiLoader", 20);
  ...
  if (!Number.isInteger(deviceLimit) || deviceLimit < 1 || deviceLimit > MAX_DEVICE_LIMIT) {
    return res.status(400).json({ success: false, message: `Device limit must be 1-${MAX_DEVICE_LIMIT}.` });
  }

  if (!Number.isInteger(quantity) || quantity < 1 || quantity > MAX_KEY_QUANTITY) {
  ...
  if (db.pricing[duration]) {
    ...
    if (!Number.isFinite(durationHours) || durationHours <= 0 || durationHours > 87600) {
  ...
  for (let i = 0; i < quantity; i++) {
    const key = {
      id: randomId("key"),
      key: ensureUniqueKey(prefix, duration, durationLabel),
...
app.get("/api/settings", auth, (req, res) => {
  ...
  res.json({ success: true, settings: { username: req.user.username, usernameChangeCount: Number(req.user.usernameChangeCount || 0), usernameChangeRemaining: Math.max(0, 1 - Number(req.user.usernameChangeCount || 0)), passwordResetAttempts: Number(req.user.passwordResetAttempts || 0), passwordResetRemaining: Math.max(0, MAX_RESET_ATTEMPTS - Number(req.user.passwordResetAttempts || 0)), passwordResetWindow: resetWindowLabel(req.user.role), panelDeviceResetUsed: Number(req.user.panelDeviceResetCount || 0), panelDeviceResetRemaining: Math.max(0, MAX_PANEL_DEVICE_RESETS - Number(req.user.panelDeviceResetCount || 0)), theme: "local" }, role: req.user.role });
});
...
<style>
:root{--bg:#0b0e14;--panel:#121722;--panel2:#171d29;--line:#283140;--text:#f2f4f8;--muted:#929bad;--accent:#f4cf70;--danger:#e66c74;--ok:#68cf98;--sidebar:250px;--shadow:0 18px 50px rgba(0,0,0,.28)}
...
.app-shell{min-height:100vh}.side-overlay{display:none}.side-overlay.open{display:block;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:45}.sidebar{position:fixed;left:0;top:0;bottom:0;width:var(--sidebar);background:#0f131b;border-right:1px solid var(--line);padding:20px 14px;z-index:50;display:flex;flex-direction:column}.side-user{padding:15px 12px;margin:18px 0 12px;background:var(--panel);border:1px solid var(--line);border-radius:12px}.side-nav{display:grid;gap:6px}.navbtn{width:100%;text-align:left;background:transparent;color:#cfd5df;border:1px solid transparent;border-radius:9px;padding:11px 12px}.navbtn:hover,.navbtn.active{background:#1a202c;border-color:var(--line);color:#fff}.side-bottom{margin-top:auto;display:grid;gap:7px}.main{margin-left:var(--sidebar);min-height:100vh}.topbar{height:72px;padding:0 24px;border-bottom:1px solid var(--line);background:rgba(11,14,20,.94);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:30}.mobile-menu{display:none}.layout{max-width:1450px;margin:auto;padding:24px}.page-head{display:flex;justify-content:space-between;align-items:center;gap:14px;margin-bottom:20px}.page-head h1{margin:0 0 5px;font-size:28px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:18px;box-shadow:var(--shadow)}.stat .label{color:var(--muted)}.stat b{display:block;font-size:29px;margin-top:9px}.toolbar{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:9px;margin-bottom:14px}.table-wrap{overflow:auto}.table{width:100%;border-collapse:collapse;min-width:920px}.table th,.table td{padding:12px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}.table th{color:#aeb7c7}.status-active{color:#68cf98;font-weight:700}.status-blocked{color:#e66c74;font-weight:700}.bracket{background:#fff;color:#111;border:1px solid #d7dbe2;border-radius:9px;padding:11px}.list-box{display:grid;margin-top:5px}.list-box button{padding:10px;text-align:left;border:1px solid var(--line);background:var(--panel2);color:var(--text)}.badge{display:inline-block;padding:4px 9px;border-radius:99px;background:#29303b;font-size:12px}.badge.ok{color:#a9ebc6}.badge.bad{color:#ffb7bd}.actions{display:flex;flex-wrap:wrap;gap:6px}.primary,.action,.danger{padding:10px 14px;border-radius:9px;border:1px solid transparent}.primary{background:var(--ok);color:#08150d;font-weight:750}.action{background:#202633;color:var(--text);border-color:#303949}.danger{background:#b83b46;color:#fff;border-color:#e15a65;font-weight:750}.full{width:100%}.field{margin:13px 0}.field label{display:block;margin-bottom:7px;color:#ccd3df}.field-input,input,select,textarea{width:100%;background:#0d1118;color:var(--text);border:1px solid var(--line);border-radius:9px;padding:11px;outline:none}.field-input:focus,input:focus,select:focus,textarea:focus{border-color:#657084}.row{display:flex;gap:9px;align-items:center}.wrap{flex-wrap:wrap}.empty{text-align:center;padding:40px;color:var(--muted)}.key{font-family:monospace;font-size:13px}.login-wrap{min-height:100vh;display:grid;place-items:center;padding:20px}.login-logo{text-align:center;margin-bottom:8px}.login-logo img{max-width:110px;max-height:70px;object-fit:contain}.side-logo{display:flex;align-items:center;gap:8px}.side-logo img{width:28px;height:28px;object-fit:contain}.password-wrap{display:flex;gap:7px}.password-wrap input{flex:1}.eye-btn{min-width:58px;border:1px solid var(--line);background:#202633;color:var(--text);border-radius:9px;padding:0 10px}.login{width:min(430px,100%);background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:30px;box-shadow:var(--shadow)}.error{color:#ffb4ba;background:#351d21;border:1px solid #63333a;padding:10px;border-radius:8px;margin:10px 0}.flash{margin-bottom:14px;padding:12px 15px;border-radius:9px;font-weight:700;border:1px solid}.flash.success{color:#63d493;background:#102a1d;border-color:#2f744b}.flash.error{color:#ff6f79;background:#35191e;border-color:#74343b}.toast{position:fixed;right:18px;bottom:18px;background:#202733;border:1px solid var(--line);padding:12px 15px;border-radius:9px;z-index:100}.modal-bg{position:fixed;inset:0;background:rgba(0,0,0,.76);display:grid;place-items:center;padding:18px;z-index:90}.modal{width:min(650px,100%);max-height:90vh;overflow:auto;background:var(--panel);border:1px solid var(--line);border-radius:15px;padding:20px}.modal h2{margin-top:0}.setting-row,.pricing-row{display:flex;justify-content:space-between;gap:15px;align-items:center;padding:13px 0;border-bottom:1px solid var(--line)}.settings-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.switch{position:relative;width:48px;height:26px}.switch input{display:none}.switch span{position:absolute;inset:0;background:#343b49;border-radius:99px}.switch span:after{content:"";position:absolute;width:20px;height:20px;left:3px;top:3px;background:#fff;border-radius:50%;transition:.15s}.switch input:checked+span{background:#5c8b6f}.switch input:checked+span:after{transform:translateX(22px)}
@media(max-width:900px){:root{--sidebar:0px}.sidebar{display:none}.sidebar.open{display:flex;width:270px;box-shadow:20px 0 60px rgba(0,0,0,.5)}.main{margin-left:0}.mobile-menu{display:inline-flex}.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.settings-grid,.grid2{grid-template-columns:1fr}.toolbar{grid-template-columns:1fr 1fr}.toolbar input{grid-column:1/-1}.topbar{padding:0 15px}}
...
</style></head><body><div id="app"></div><div id="toast" class="toast hidden"></div>
<script>
let token=localStorage.getItem('lexi_token')||'';let ownerToken=localStorage.getItem('lexi_owner_token')||'';let currentUser=null;let pricing={};let currentPage=localStorage.getItem('lexi_page')||'dashboard';window.__panelFlash=null;window.__generatedKeys=null;let booting=false;
function esc(v){return String(v??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]))}
function getPanelDeviceId(){let id=localStorage.getItem('lexi_panel_device_id');if(!id){id=(crypto.randomUUID?crypto.randomUUID():'dev-'+Date.now()+'-'+Math.random().toString(36).slice(2));localStorage.setItem('lexi_panel_device_id',id)}return id}
function setFlash(message,type='success'){window.__panelFlash={message,type};const html='<div class="flash '+(type==='error'?'error':'success')+'">'+esc(message)+'</div>';const a=document.getElementById('flashArea');if(a)a.innerHTML=html;const l=document.getElementById('loginFlash');if(l)l.innerHTML=html;}
...
function keySuccessCard(keys,title='Key Generated Successfully'){if(!keys||!keys.length)return '';return '<div class="card" style="margin-bottom:14px;border-color:#2f744b"><div class="flash success" style="margin:0 0 10px">'+esc(title)+'</div>'+keys.map(k=>'<div style="padding:10px 0;border-bottom:1px solid var(--line)"><div class="row" style="justify-content:space-between;gap:8px"><b class="key">'+esc(k.key)+'</b><button class="action" onclick="copyText(\''+esc(k.key)+'\')">Copy</button></div><small>Duration: '+esc(k.durationLabel||k.duration||'')+' · Devices: '+Number(k.deviceLimit||1)+' · Status: '+esc(k.status||'active')+' · Expiry: '+esc(k.expiryAt?fmt(k.expiryAt):'Not Started')+'</small></div>').join('')+'</div>'}
...
function renderLogin(){const f=consumeFlash();document.getElementById('app').innerHTML='<div class="login-wrap"><div class="login"><div id="loginFlash">'+f+'</div><div class="login-logo"><img src="/assets/lexi-logo.png" alt="Lexi Loader" onerror="this.style.display='none'"></div><div class="brand">Lexi <span>Loader</span></div><p class="muted">Reseller Management Panel</p><form onsubmit="login(event)"><div class="field"><label>Username</label><input id="loginUser" autocomplete="username" required></div><div class="field"><label>Password</label><div class="password-wrap"><input id="loginPass" type="password" autocomplete="current-password" required><button type="button" class="eye-btn" onclick="togglePassword('loginPass',this)" aria-label="Show password">Show</button></div></div><div class="field"><label>Session</label><select id="sessionMode"><option value="24">24 Hours</option><option value="720">30 Days</option></select></div><div id="loginErr" class="error hidden"></div><button class="primary full" style="margin-top:14px" type="submit">Sign in</button></form><button id="resetDeviceBtn" class="action full hidden" style="margin-top:9px" onclick="resetDeviceModal()">Reset Panel Device</button><button class="action full" style="margin-top:9px" onclick="renderRegister()">Register with Referral Code</button><button class="action full" style="margin-top:9px" onclick="resetPasswordModal()">Reset Password</button></div></div>'}
async function login(e){e.preventDefault();const er=document.getElementById('loginErr');er.classList.add('hidden');try{const r=await fetch('/api/auth/login',{credentials:'same-origin',method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:document.getElementById('loginUser').value,password:document.getElementById('loginPass').value,rememberMe:document.getElementById('sessionMode').value==='720',deviceId:getPanelDeviceId()})});const d=await r.json();if(!r.ok)throw new Error(d.message||'Login failed');setToken(d.token);currentUser=d.user;await shell();await page(currentPage)}catch(x){er.textContent=x.message;er.classList.remove('hidden');if(String(x.message||'').toLowerCase().includes('another device'))document.getElementById('resetDeviceBtn')?.classList.remove('hidden');setFlash(x.message,'error')}}
function resetDeviceModal(){modal('<h2>Reset Panel Device</h2><p class="muted">This authorizes the current device for this account. Maximum 3 device resets.</p><div class="field"><label>Username</label><input id="deviceResetUser" autocomplete="username"></div><div class="field"><label>Password</label><div class="password-wrap"><input id="deviceResetPass" type="password" autocomplete="current-password"><button type="button" class="eye-btn" onclick="togglePassword('deviceResetPass',this)">Show</button></div></div><div id="deviceResetErr" class="error hidden"></div><button class="primary" onclick="resetPanelDevice()">Reset Device</button> <button class="action" onclick="closeModal()">Cancel</button>')}
async function resetPanelDevice(){const er=document.getElementById('deviceResetErr');er.classList.add('hidden');try{const r=await fetch('/api/auth/reset-device',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:document.getElementById('deviceResetUser').value,password:document.getElementById('deviceResetPass').value,deviceId:getPanelDeviceId()})});const d=await r.json();if(!r.ok)throw new Error(d.message||'Device reset failed');closeModal();setFlash('Device reset successfully. This device is now authorized.','success');renderLogin()}catch(e){er.textContent=e.message;er.classList.remove('hidden');setFlash('Device reset failed: '+e.message,'error')}}
function togglePassword(id,btn){const el=document.getElementById(id);if(!el)return;const show=el.type==='password';el.type=show?'text':'password';if(btn)btn.textContent=show?'Hide':'Show';}
function resetPasswordModal(){modal('<h2>Reset Password</h2><p class="muted">Uses username and your current registered IP. Maximum 3 resets per role window.</p><div class="field"><label>Username</label><input id="resetUser"></div><div class="field"><label>New Password</label><div class="password-wrap"><input id="resetNew" type="password" minlength="6"><button type="button" class="eye-btn" onclick="togglePassword('resetNew',this)">Show</button></div></div><div id="resetErr" class="error hidden"></div><button class="primary" onclick="resetPasswordSave()">Reset Password</button> <button class="action" onclick="closeModal()">Cancel</button>')}
...
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';let items=navItem('dashboard','Dashboard')+navItem('keys','Keys');if(owner)items+=navItem('users','Users');items+=navItem('referrals','Referrals');if(owner||admin)items+=navItem('transactions','Transactions')+navItem('activity','Activity');if(owner)items+=navItem('audit','Audit Logs')+navItem('settings','Settings')+navItem('online','Online');else items+=navItem('settings','Settings');return '<div id="sideOverlay" class="side-overlay" onclick="closeSideMenu()"></div><aside id="sidebar" class="sidebar"><div class="side-logo"><img src="/assets/lexi-logo.png" alt="Lexi" onerror="this.style.display=\'none\'"><div class="brand">Lexi <span>Loader</span></div></div><div class="side-user"><b>'+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">'+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+items+'</div><div class="side-bottom">'+(ownerToken&&owner?'<button class="primary full" onclick="returnOwner()">Return to Owner</button>':'')+'<button class="action full" onclick="logout()">Logout</button></div></aside>'}
...
async function shell(){document.getElementById('app').innerHTML='<div class="app-shell">'+sidebar()+'<section class="main"><header class="topbar"><div class="row"><button class="action mobile-menu" onclick="document.getElementById(\'sidebar\').classList.toggle(\'open\')">Menu</button><div><b>'+esc(currentUser.role.toUpperCase())+'</b><div class="muted">'+esc(currentUser.username)+'</div></div></div><div class="row"><button class="action" onclick="toggleTheme()">Theme</button><button class="action" onclick="passwordModal()">Password</button></div></header><main class="layout"><div id="flashArea">'+consumeFlash()+'</div><div id="content"></div></main></section></div>'}
async function boot(){if(booting)return;booting=true;try{if(!token){renderLogin();return}const m=await api('/api/auth/me');currentUser=m.user;await shell();await page(currentPage)}catch(e){if(e.status===401){localStorage.removeItem('lexi_token');token='';currentUser=null;renderLogin()}else{if(!currentUser){renderLogin()}else{await shell();document.getElementById('content').innerHTML='<div class="card"><h2>Connection problem</h2><p class="muted">Your session is still saved. Retry without logging out.</p><button class="primary" onclick="boot()">Retry</button></div>'}}}finally{booting=false}}
async function page(name){const changed=name!==currentPage;if(changed){window.__generatedKeys=null;window.__panelFlash=null;const fa=document.getElementById('flashArea');if(fa)fa.innerHTML='';}currentPage=name;localStorage.setItem('lexi_page',name);closeSideMenu();document.querySelectorAll('.navbtn').forEach(b=>b.classList.toggle('active',b.dataset.page===name));if(!currentUser)return;if(name==='dashboard')return dashboard();if(name==='keys')return keysPage();if(name==='users'&&currentUser.role==='owner')return usersPage();if(name==='referrals')return referralsPage();if(name==='transactions'&&(currentUser.role==='owner'||currentUser.role==='admin'))return transactionsPage();if(name==='activity'&&(currentUser.role==='owner'||currentUser.role==='admin'))return activityPage();if(name==='audit'&&currentUser.role==='owner')return auditPage();if(name==='settings')return settingsPage();if(name==='online'&&currentUser.role==='owner')return onlinePage();if(name==='settings'||name==='online')return dashboard()}
function stat(label,value){return '<div class="card stat"><div class="label">'+esc(label)+'</div><b>'+esc(value)+'</b></div>'}
async function dashboard(){try{const d=await api('/api/dashboard');const s=d.stats||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Dashboard</h1><div class="muted">License and account overview</div></div><button class="primary" onclick="generateModal()">Generate Key</button></div><div class="grid">'+stat('Active Keys',s.activeKeys||0)+stat('Expired Keys',s.expiredKeys||0)+stat('Blocked Keys',s.blockedKeys||0)+stat(currentUser.role==='owner'?'Users':'My Keys',s.users||s.activeKeys||0)+'</div><div class="grid2" style="margin-top:14px">'+stat('Balance','₹'+Number(s.balance||0).toFixed(2))+stat('Resellers',s.resellers||0)+'</div><div class="card" style="margin-top:14px"><h3>Loader Status</h3><p><span class="badge '+(d.settings?.loaderStatus?'ok':'bad')+'">'+(d.settings?.loaderStatus?'ONLINE':'MAINTENANCE')+'</span></p><p class="muted">'+esc(d.settings?.loaderStatus?'Loader verification is enabled.':(d.settings?.maintenanceMsg||'Loader disabled.'))+'</p></div>'}catch(e){toast(e.message)}}
async function keysPage(){try{const d=await api('/api/keys');pricing=(await api('/api/pricing')).pricing||{};document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Keys</h1><div class="muted">Manage licenses and device bindings</div></div><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="card"><div class="toolbar"><input id="keySearch" placeholder="Search license / HWID"><select id="keyStatus"><option value="all">All Status</option><option value="active">Active</option><option value="blocked">Blocked</option><option value="expired">Expired</option><option value="deleted">Deleted</option></select><select id="keyDevice"><option value="all">All Devices</option><option value="bound">Bound</option><option value="unbound">Unbound</option></select><button class="action" onclick="loadKeys()">Search</button></div><div id="keysTable"></div></div>';window.__keys=d.keys||[];renderKeys()}catch(e){setFlash('Failed: '+e.message,'error');renderFlashOnly()}}
function renderKeys(){let q=(document.getElementById('keySearch')?.value||'').toLowerCase(),st=document.getElementById('keyStatus')?.value||'all',dv=document.getElementById('keyDevice')?.value||'all';let rows=(window.__keys||[]).filter(k=>{let ok=!q||String(k.key).toLowerCase().includes(q)||String(k.hwid||'').toLowerCase().includes(q);let ss=st==='all'||k.status===st;let bound=Array.isArray(k.devices)&&k.devices.length>0;let dd=dv==='all'||(dv==='bound'?bound:!bound);return ok&&ss&&dd});if(!rows.length){document.getElementById('keysTable').innerHTML='<div class="empty">No keys found.</div>';return}document.getElementById('keysTable').innerHTML='<div class="table-wrap"><table class="table"><thead><tr><th>License</th><th>Status</th><th>Duration</th><th>Devices</th><th>Expiry</th><th>Creator</th><th>Actions</th></tr></thead><tbody>'+rows.map(k=>'<tr><td><b class="key">'+esc(k.key)+'</b><br><small>'+esc(k.hwid||'Unbound')+'</small></td><td><span class="badge '+(k.status==='active'?'ok':'bad')+'">'+esc(k.status)+'</span></td><td>'+esc(k.durationLabel||k.duration||'')+'</td><td>'+((k.devices||[]).length)+' / '+Number(k.deviceLimit||1)+'</td><td>'+esc(k.expiryAt?fmt(k.expiryAt):'Not Started')+'</td><td>'+esc(k.creatorUsername||k.creatorId||'')+'</td><td><div class="actions"><button class="action" onclick="copyText(\''+esc(k.key)+'\')">Copy</button><button class="action" onclick="keyAction(\''+esc(k.id)+'\',\'reset\')">Reset</button><button class="action" onclick="keyAction(\''+esc(k.id)+'\',\'block\')">'+(k.status==='blocked'?'Unblock':'Block')+'</button><button class="danger" onclick="keyAction(\''+esc(k.id)+'\',\'delete\')">Delete</button></div></td></tr>').join('')+'</tbody></table></div>'}
...
function generateModal(){let opts=Object.keys(pricing).map(k=>'<option value="'+esc(k)+'">'+esc(pricing[k].label)+' — ₹'+Number(pricing[k].price).toFixed(2)+'</option>').join('');let custom=['owner','admin'].includes(currentUser.role);let price=currentUser.role==='owner'?'<div class="field"><label>Custom Price</label><input id="customPrice" type="number" min="0" step="0.01" value="0"></div>':'';modal('<h2>Generate Keys</h2><div class="field"><label id="durationHeading">Duration</label><select id="genDuration" onchange="customDurationToggle()">'+opts+(custom?'<option value="custom">Custom</option>':'')+'</select></div><div id="customFields" class="hidden"><div class="field"><label>Custom Key</label><input id="customHours" type="number" min="1" placeholder="Duration in hours"></div>'+price+'<div class="field"><label>Label</label><input id="customLabel" placeholder="Custom Key"></div></div><div class="field"><label>Device Limit</label><input id="genDevices" type="number" min="1" max="9999" value="1"></div><div class="field"><label>Key Count</label><input id="genCount" type="number" min="1" max="1000" value="1"></div><div class="field"><label>Prefix</label><input id="genPrefix" value="Lexi-Loader"></div><div id="genErr" class="error hidden"></div><button class="primary" onclick="generateSave()">Generate</button> <button class="action" onclick="closeModal()">Cancel</button>')}
...
async function generateSave(){const er=document.getElementById('genErr');try{const duration=document.getElementById('genDuration').value;const body={duration,deviceLimit:Number(document.getElementById('genDevices').value),quantity:Number(document.getElementById('genCount').value),prefix:document.getElementById('genPrefix').value};if(duration==='custom'){body.customHours=Number(document.getElementById('customHours').value);body.customLabel=document.getElementById('customLabel').value;if(currentUser.role==='owner')body.customPrice=Number(document.getElementById('customPrice').value||0)}const d=await api('/api/keys/generate',{method:'POST',body:JSON.stringify(body)});closeModal();window.__generatedKeys=d.keys||[];setFlash(d.message||'Keys generated successfully.','success');await keysPage()}catch(e){er.textContent=e.message;er.classList.remove('hidden');setFlash('Key generation failed: '+e.message,'error')}}
...
function renderUsers(){const q=(document.getElementById('userSearch')?.value||'').toLowerCase(),role=document.getElementById('userRole')?.value||'all',status=document.getElementById('userStatus')?.value||'all';const rows=(window.__users||[]).filter(u=>(!q||String(u.username).toLowerCase().includes(q)||String(u.id).toLowerCase().includes(q))&&(role==='all'||u.role===role)&&(status==='all'||u.status===status));document.getElementById('usersTable').innerHTML='<div class="table-wrap"><table class="table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Balance</th><th>Created</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(userRow).join(''):'<tr><td colspan="6" class="empty">No users found.</td></tr>')+'</tbody></table></div>'}
async function loadUsers(){try{const d=await api('/api/users');window.__users=d.users||[];renderUsers()}catch(e){setFlash('Users load failed: '+e.message,'error');renderFlashOnly()}}
function userRow(u){const self=u.id===currentUser.id;let actions='';if(!self){actions+='<button class="action" onclick="toggleUser(\''+esc(u.id)+'\',\''+(u.status==='active'?'blocked':'active')+'\')">'+(u.status==='active'?'Block':'Unblock')+'</button>';actions+='<button class="action" onclick="manageUser(\''+esc(u.id)+'\')">Manage</button>';actions+='<button class="danger" onclick="toggleUser(\''+esc(u.id)+'\',\'deleted\')">Delete</button>';if(u.role!=='owner')actions+='<button class="action" onclick="enterPanel(\''+esc(u.id)+'\')">Enter</button>'}else actions='<span class="muted">Current account</span>';return '<tr><td><b>'+esc(u.username)+'</b><br><small>'+esc(u.id)+'</small></td><td>'+esc(u.role)+'</td><td><span class="badge '+(u.status==='active'?'ok':'bad')+'">'+esc(u.status)+'</span></td><td>₹'+Number(u.balance||0).toFixed(2)+'</td><td>'+esc(fmt(u.createdAt))+'</td><td><div class="actions">'+actions+'</div></td></tr>'}
async function manageUser(id){try{const d=await api('/api/users/'+encodeURIComponent(id));const u=d.user;modal('<h2>Manage User</h2><div class="field"><label>Username</label><div class="bracket">'+esc(u.username)+'</div></div><div class="field"><label>Role</label><div class="bracket">'+esc(u.role.toUpperCase())+'</div></div><div class="field"><label>Status</label><div class="bracket '+(u.status==='active'?'status-active':'status-blocked')+'">'+esc(u.status)+'</div></div><div class="field"><label>Balance</label><div class="bracket">₹'+Number(u.balance||0).toFixed(2)+'</div></div><div class="field"><label>Created</label><div class="bracket">'+esc(fmt(u.createdAt))+'</div></div><button class="action" onclick="closeModal()">Close</button>')}catch(e){setFlash('Manage failed: '+e.message,'error');renderFlashOnly()}}
function createUserModal(){const owner=currentUser.role==='owner';modal('<h2>Create '+(owner?'Account':'Reseller')+'</h2><div class="field"><label>Username</label><input id="newUser" autocomplete="off"></div><div class="field"><label>Password</label><div class="password-wrap"><input id="newPass" type="password"><button type="button" class="eye-btn" onclick="togglePassword('newPass',this)">Show</button></div></div>'+(owner?'<div class="field"><label>Role</label><select id="newRole"><option value="admin">Admin</option><option value="reseller">Reseller</option><option value="owner">Owner</option></select></div>':'<input id="newRole" type="hidden" value="reseller">')+'<div class="field"><label>Starting Balance</label><input id="newBal" type="number" min="0" step="0.01" value="0"></div><div id="createErr" class="error hidden"></div><button class="primary" onclick="createUserSave()">Create</button> <button class="action" onclick="closeModal()">Cancel</button>')}
async function createUserSave(){const er=document.getElementById('createErr');try{const d=await api('/api/users/create',{method:'POST',body:JSON.stringify({username:document.getElementById('newUser').value,password:document.getElementById('newPass').value,role:document.getElementById('newRole').value,balance:Number(document.getElementById('newBal').value||0)})});closeModal();setFlash('Account created successfully.','success');usersPage()}catch(e){er.textContent=e.message;er.classList.remove('hidden')}}
async function toggleUser(id,status){if(!confirm(status==='deleted'?'Delete this account?':'Change account status?'))return;try{await api('/api/users/'+encodeURIComponent(id)+'/status',{method:'POST',body:JSON.stringify({status})});setFlash('Account updated successfully.','success');usersPage()}catch(e){toast(e.message)}}
...
async function referralsPage(){try{const d=await api('/api/referrals');let rows=d.referrals||[];if(currentUser.role==='admin')rows=rows.filter(r=>r.creatorId===currentUser.id);document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Referrals</h1><div class="muted">Referral codes and referred user details</div></div><button class="primary" onclick="createReferralModal()">Create Referral</button></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Referral Code</th><th>Creator</th><th>Used</th><th>Limit</th><th>Expiry</th><th>Status</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(r=>'<tr><td><b>'+esc(r.code)+'</b></td><td>'+esc(r.creatorUsername||r.creatorId||'')+'</td><td>'+Number(r.usedCount||0)+'</td><td>'+(Number(r.usageLimit||0)>0?Number(r.usageLimit):'Unlimited')+'</td><td>'+esc(r.expiresAt?fmt(r.expiresAt):'Never')+'</td><td><span class="status-text '+(r.status==='active'?'status-active':'status-blocked')+'">'+esc(r.status)+'</span></td><td class="actions"><button class="action" onclick="manageReferral(\''+esc(r.id)+'\')">Manage</button><button class="danger" onclick="deleteReferral(\''+esc(r.id)+'\')">Delete</button></td></tr>').join(''):'<tr><td colspan="7" class="empty">No referrals yet.</td></tr>')+'</tbody></table></div></div>'}catch(e){setFlash('Referrals load failed: '+e.message,'error');renderFlashOnly()}}
async function manageReferral(id){try{const d=await api('/api/referrals');const r=(d.referrals||[]).find(x=>x.id===id);if(!r)throw new Error('Referral not found.');modal('<h2>Manage Referral</h2><div class="field"><label>Referral Code</label><div class="bracket">'+esc(r.code)+'</div></div><div class="field"><label>User Details</label><div class="bracket">Creator: '+esc(r.creatorUsername||r.creatorId||'')+'<br>Used: '+Number(r.usedCount||0)+' / '+(Number(r.usageLimit||0)>0?Number(r.usageLimit):'Unlimited')+'<br>Role: '+esc(String(r.role||'reseller').toUpperCase())+'<br>Starting Balance: ₹'+Number(r.startingBalance||0).toFixed(2)+'</div></div><div class="field"><label>Status</label><div class="bracket '+(r.status==='active'?'status-active':'status-blocked')+'">'+esc(r.status)+'</div></div><div class="field"><label>Expiry</label><div class="bracket">'+esc(r.expiresAt?fmt(r.expiresAt):'Never')+'</div></div><div class="actions"><button class="action" onclick="toggleReferral(\''+esc(r.id)+'\')">'+(r.status==='active'?'Block':'Unblock')+'</button><button class="danger" onclick="deleteReferral(\''+esc(r.id)+'\')">Delete</button><button class="action" onclick="closeModal()">Close</button></div>')}catch(e){setFlash('Manage referral failed: '+e.message,'error');renderFlashOnly()}}
...
async function passwordModal(){modal('<h2>Change Password</h2><div class="field"><label>New Password</label><input id="newPassword" type="password" minlength="6"></div><div id="pwErr" class="error hidden"></div><button class="primary" onclick="changePassword()">Save</button> <button class="action" onclick="closeModal()">Cancel</button>')}
...
async function onlinePage(){try{const d=await api('/api/settings');const s=d.settings||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Online</h1><div class="muted">App server control and online system</div></div></div><div class="settings-grid"><div class="card"><h3>Full System</h3><div class="setting-row"><div><b>System Online</b><small>Turn the loader/app verification system on or off</small></div><label class="switch"><input id="sysOnline" type="checkbox" '+(s.systemOnline!==false?'checked':'')+'><span></span></label></div><div class="field"><label>Online / Offline Message</label><textarea id="onlineMsg" rows="4">'+esc(s.onlineMessage||'')+'</textarea></div><button class="primary" onclick="saveOnline()">Save System</button></div><div class="card"><h3>App Message</h3><div class="setting-row"><div><b>Show Popup</b><small>Display owner message in loader/app</small></div><label class="switch"><input id="noticeEnabled" type="checkbox" '+(s.appNoticeEnabled?'checked':'')+'><span></span></label></div><div class="field"><label>Message</label><textarea id="appNotice" rows="8" placeholder="Add 5-10 lines for update, maintenance, new version, etc.">'+esc(s.appNotice||'')+'</textarea></div><div class="field"><label>Message Type</label><select id="noticeType"><option value="info" '+(s.appNoticeType==='info'?'selected':'')+'>Info</option><option value="update" '+(s.appNoticeType==='update'?'selected':'')+'>Update</option><option value="maintenance" '+(s.appNoticeType==='maintenance'?'selected':'')+'>Maintenance</option></select></div><button class="primary" onclick="saveOnline()">Save App Message</button></div></div>'}catch(e){toast(e.message)}}
...
async function savePanel(){try{await api('/api/settings/update',{method:'POST',body:JSON.stringify({panelName:document.getElementById('setName').value,supportLink:document.getElementById('setSupport').value,sessionTimeoutHours:Number(document.getElementById('setSession').value)})});toast('Panel settings saved')}catch(e){toast(e.message)}}
async function saveSecurity(){try{await api('/api/settings/update',{method:'POST',body:JSON.stringify({ipGeoLockEnabled:document.getElementById('setGeo').checked})});toast('Security setting saved')}catch(e){toast(e.message)}}
...
function renderRegister(){const ref=new URLSearchParams(location.search).get('ref')||'';document.getElementById('app').innerHTML='<div class="login-wrap"><div class="login"><div class="login-logo"><img src="/assets/lexi-logo.png" alt="Lexi Loader" onerror="this.style.display='none'"></div><div class="brand">Lexi <span>Loader</span></div><p class="muted">Create Reseller account using referral</p><form onsubmit="registerAccount(event)"><div class="field"><label>Username</label><input id="regUser" autocomplete="username" required></div><div class="field"><label>Password</label><div class="password-wrap"><input id="regPass" type="password" autocomplete="new-password" required><button type="button" class="eye-btn" onclick="togglePassword('regPass',this)">Show</button></div></div><div class="field"><label>Referral Code</label><input id="regRef" value="'+esc(ref)+'" placeholder="Enter referral code" autocomplete="off" required></div><div id="regErr" class="error hidden"></div><button class="primary full" type="submit">Create Reseller Account</button></form><button class="action full" style="margin-top:9px" onclick="location.href='/'">Back to Login</button></div></div>'}
...
window.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal()});applyTheme();
(async()=>{if(location.pathname==='/register'){renderRegister();return}await boot()})();
</script></body></html>`;


app.get("/", (req, res) => res.type("html").send(PANEL_HTML));
app.get("/panel", (req, res) => res.type("html").send(PANEL_HTML));
app.get("/register", (req, res) => res.type("html").send(PANEL_HTML));
