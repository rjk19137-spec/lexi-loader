...
app.use((req,res,next)=>{
  ...
  res.setHeader("Content-Security-Policy","default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'");
...
// Defense-in-depth API controls. Loader heartbeat/config/version remain lightweight.
const API_RATE_WINDOW_MS = 60 * 1000;
const API_RATE_LIMIT = 120;
const apiRateState = new Map();
function rateLimitApi(req, res, next) {
  if (!req.path.startsWith("/api/")) return next();
  if (["/loader/heartbeat", "/loader/config", "/loader/version", "/health"].includes(req.path)) return next();
  const key = `${clientIp(req)}:${req.method}:${req.path}`;
  const t = Date.now();
  let row = apiRateState.get(key);
  if (!row || t - row.startedAt >= API_RATE_WINDOW_MS) row = { startedAt: t, count: 0 };
  row.count += 1; apiRateState.set(key, row);
  if (row.count > API_RATE_LIMIT) return res.status(429).json({ success: false, code: "RATE_LIMITED", message: "Too many requests. Please try again later.", requestId: req.requestId });
  if (apiRateState.size > 5000) for (const [k,v] of apiRateState) if (t - v.startedAt >= API_RATE_WINDOW_MS) apiRateState.delete(k);
  next();
}
function sameOriginForUnsafeApi(req) {
  if (!["POST","PUT","PATCH","DELETE"].includes(req.method)) return true;
  const origin = String(req.headers.origin || "").trim();
  if (!origin) return true;
  const host = String(req.headers.host || "").trim();
  const proto = process.env.NODE_ENV === "production" ? "https" : (String(req.headers["x-forwarded-proto"] || "http").split(",")[0].trim());
  return origin === `${proto}://${host}`;
}
app.use("/api", rateLimitApi);
app.use("/api", (req, res, next) => {
  if (!sameOriginForUnsafeApi(req)) return res.status(403).json({ success: false, code: "ORIGIN_REJECTED", message: "Request origin is not allowed.", requestId: req.requestId });
  next();
});
app.use("/api/loader", (req, res, next) => {
  if (req.method !== "GET" && Number(req.headers["content-length"] || 0) > MAX_LOADER_BODY) return res.status(413).json({ success: false, code: "REQUEST_TOO_LARGE", message: "Request body is too large.", requestId: req.requestId });
  next();
});
...
const LOADER_HEARTBEAT_GRACE_MS = 2 * 60 * 1000;
const MAX_LOADER_BODY = 32 * 1024;
const LOADER_API_VERSION = "1";
const LOADER_MIN_VERSION = "1.0.0";
const LOADER_CURRENT_VERSION = "1.0.0";
...
function clearSessionCookie(res) {
  res.setHeader("Set-Cookie", `lexi_session=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax${process.env.NODE_ENV === "production" ? "; Secure" : ""}`);
}
...
function initialDatabase() {
  return {
    ...
    pricing: { ...PRICING },
    users: [], referrals: [], keys: [], devices: [], transactions: [], auditLogs: [], sessions: [], rateLimits: [], loaderSessions: []
  };
}
...
function migrateDatabase() {
  ...
  db.loaderSessions = Array.isArray(db.loaderSessions) ? db.loaderSessions : [];
  for (const s of db.loaderSessions) {
    if (!s.createdAt) s.createdAt = now();
    if (!s.lastSeenAt) s.lastSeenAt = s.createdAt;
    if (!s.expiresAt) s.expiresAt = new Date(Date.parse(s.createdAt) + LOADER_SESSION_TTL_MS).toISOString();
  }
  for (const s of db.sessions) {
    if (!s.tokenHash && s.token) s.tokenHash = loaderTokenHash(s.token);
    delete s.token;
  }
...
function getSession(req) {
  ...
  const tokenHash = loaderTokenHash(token);
  const session = db.sessions.find(s => safeEqual(s.tokenHash || "", tokenHash));
...
function ensureOwner() {
  ...
  if (process.env.NODE_ENV === "production" && !process.env.LEXI_OWNER_PASSWORD) {
    throw new Error("LEXI_OWNER_PASSWORD must be set before first production startup.");
  }
...
function cleanupLoaderSessions() {
  const cutoff = Date.now() - LOADER_HEARTBEAT_GRACE_MS;
  db.loaderSessions = db.loaderSessions.filter(s => {
    const exp = Date.parse(s.expiresAt || 0);
    const seen = Date.parse(s.lastSeenAt || s.createdAt || 0);
    return Number.isFinite(exp) && exp > Date.now() && Number.isFinite(seen) && seen >= cutoff;
  });
}

function loaderTokenHash(token) {
  return crypto.createHash("sha256").update(String(token)).digest("hex");
}

function findLoaderSession(token) {
  if (!token) return null;
  const hash = loaderTokenHash(token);
  return db.loaderSessions.find(s => safeEqual(s.tokenHash, hash)) || null;
}

function publicLoaderLicense(key) {
  return {
    id: key.id,
    ...
    deviceLimit: Number(key.deviceLimit || 1),
    devicesUsed: db.devices.filter(d => d.keyId === key.id).length,
    gamePackage: key.gamePackage || ""
  };
}

function loaderVersionAllowed(version) {
  const v = sanitizeString(version || "", 32);
  if (!v) return false;
  return v === LOADER_CURRENT_VERSION;
}

function loaderGameAllowed(key, packageName) {
  const expected = sanitizeString(key.gamePackage || "", 200);
  const actual = sanitizeString(packageName || "", 200);
  return !expected || !actual || expected === actual;
}

function loaderSessionResponse(session, key, responseToken = null) {
  return {
    ...(responseToken ? { token: responseToken } : {}),
    expiresAt: session.expiresAt,
    heartbeatAfterSeconds: 45,
    license: publicLoaderLicense(key),
    loader: {
      apiVersion: LOADER_API_VERSION,
      version: LOADER_CURRENT_VERSION,
      minVersion: LOADER_MIN_VERSION,
...
function denyLoader(res, status, code, message, requestId) {
  return res.status(status).json({ success: false, code, message, requestId });
}

function validateLoaderRequest(req) {
  const keyValue = sanitizeString(req.body.key, 150);
  ...
  const version = sanitizeString(req.body.loaderVersion, 32);
  if (!keyValue || !hwid) return { error: ["INVALID_REQUEST", "Key and device are required."] };
  if (!loaderVersionAllowed(version)) return { error: ["VERSION_UNSUPPORTED", "Loader version is not supported."] };
  return { keyValue, hwid, deviceName, packageName, version };
}

function verifyLoaderLicense(req) {
  const parsed = validateLoaderRequest(req);
  if (parsed.error) return { ok: false, code: parsed.error[0], message: parsed.error[1] };

  if (!db.settings.loaderStatus || db.settings.systemOnline === false) {
    return { ok: false, code: "LOADER_DISABLED", message: db.settings.maintenanceMsg || "Loader is disabled." };
  }

  const { keyValue, hwid, deviceName, packageName } = parsed;
  const ip = clientIp(req);
  const key = db.keys.find(k => k.key === keyValue);
  if (!key || key.status === KEY_STATUS.DELETED) return { ok: false, code: "KEY_NOT_FOUND", message: "Invalid key." };

  const status = keyStatus(key);
  if (status === KEY_STATUS.BLOCKED) return { ok: false, code: "KEY_BLOCKED", message: "Key is disabled." };
  if (status === KEY_STATUS.EXPIRED) return { ok: false, code: "KEY_EXPIRED", message: "Key has expired." };
  if (key.ipLock && key.ipLock !== ip) return { ok: false, code: "IP_LOCKED", message: "IP lock rejected this device." };

  if (key.geoLock) {
    const country = String(req.headers["x-country-code"] || "").toUpperCase();
    if (!country || country !== String(key.geoLock).toUpperCase()) {
      return { ok: false, code: "GEO_LOCKED", message: "Geo lock rejected this request." };
    }
  }

  if (!loaderGameAllowed(key, packageName)) {
    return { ok: false, code: "GAME_NOT_ALLOWED", message: "This key is not assigned to this package." };
  }
  ...
  if (!device) {
    if (devices.length >= Number(key.deviceLimit || 1)) {
      return { ok: false, code: "DEVICE_LIMIT_REACHED", message: "Key is already linked to another device." };
    }
    if (firstActivation) {
  ...
  return { ok: true, key, device, ip, hwid, packageName };
}
...
app.get("/api/loader/version", (req, res) => {
  res.json({
    ...
    requestId: req.requestId,
    apiVersion: LOADER_API_VERSION,
    currentVersion: LOADER_CURRENT_VERSION,
    minimumVersion: LOADER_MIN_VERSION,
    updateRequired: false
  });
});

/* Backward-compatible endpoint used by the existing blueprint. */
app.post("/api/loader/verify", (req, res) => {
  cleanupLoaderSessions();
  const result = verifyLoaderLicense(req);
  if (!result.ok) return denyLoader(res, result.code === "LOADER_DISABLED" ? 503 : 403, result.code, result.message, req.requestId);

  const rawToken = randomToken(32);
  const session = {
    id: randomId("ls"),
    tokenHash: loaderTokenHash(rawToken),
    keyId: result.key.id,
    deviceId: result.device.id,
    hwid: result.hwid,
    packageName: result.packageName || "",
    loaderVersion: sanitizeString(req.body.loaderVersion, 32),
    ip: result.ip,
    createdAt: now(),
    lastSeenAt: now(),
    expiresAt: new Date(Date.now() + LOADER_SESSION_TTL_MS).toISOString()
  };

  db.loaderSessions.push(session);
  audit(null, "LOADER_VERIFY", "key", result.key.id, {
    ip: result.ip,
    details: {
      requestId: req.requestId,
      hwid: result.hwid.slice(0, 12),
      deviceId: result.device.id,
      packageName: result.packageName || "",
  ...
  res.json({
    ...
    code: "VERIFIED",
    requestId: req.requestId,
    ...loaderSessionResponse(session, result.key, rawToken)
  });
});

app.post("/api/loader/heartbeat", (req, res) => {
  cleanupLoaderSessions();
  const token = sanitizeString(req.body.token, 300);
  const session = findLoaderSession(token);
  if (!session) return denyLoader(res, 401, "SESSION_INVALID", "Loader session is invalid.", req.requestId);
  if (Date.now() >= Date.parse(session.expiresAt)) {
    db.loaderSessions = db.loaderSessions.filter(s => s.id !== session.id);
    save();
    return denyLoader(res, 401, "SESSION_EXPIRED", "Loader session expired.", req.requestId);
  }

  const key = db.keys.find(k => k.id === session.keyId);
  const device = db.devices.find(d => d.id === session.deviceId && d.keyId === session.keyId);
  if (!key || !device) return denyLoader(res, 403, "AUTHORIZATION_REVOKED", "Authorization is no longer valid.", req.requestId);
  if (session.ip && session.ip !== clientIp(req)) return denyLoader(res, 401, "SESSION_IP_CHANGED", "Session network binding changed.", req.requestId);
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return denyLoader(res, 503, "LOADER_DISABLED", db.settings.maintenanceMsg || "Loader is disabled.", req.requestId);

  const status = keyStatus(key);
  if (status !== KEY_STATUS.ACTIVE) return denyLoader(res, 403, status === KEY_STATUS.EXPIRED ? "KEY_EXPIRED" : "KEY_BLOCKED", "Authorization is no longer valid.", req.requestId);

  session.lastSeenAt = now();
  device.lastSeenAt = now();
  audit(null, "LOADER_HEARTBEAT", "key", key.id, { ip: clientIp(req), details: { requestId: req.requestId, sessionId: session.id } });
  save();

  res.json({ success: true, code: "ALIVE", requestId: req.requestId, ...loaderSessionResponse(session, key) });
});
...
app.post("/api/loader/launch", (req, res) => {
  ...
  const session = findLoaderSession(token);
  if (!session) return denyLoader(res, 401, "SESSION_INVALID", "Loader session is invalid.", req.requestId);
  if (Date.now() >= Date.parse(session.expiresAt)) return denyLoader(res, 401, "SESSION_EXPIRED", "Loader session expired.", req.requestId);
  ...
  const status = keyStatus(key);
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return denyLoader(res, 503, "LOADER_DISABLED", db.settings.maintenanceMsg || "Loader is disabled.", req.requestId);
  if (status === KEY_STATUS.BLOCKED) return denyLoader(res, 403, "KEY_BLOCKED", "Key is disabled.", req.requestId);
  if (status === KEY_STATUS.EXPIRED) return denyLoader(res, 403, "KEY_EXPIRED", "Key has expired.", req.requestId);

  const packageName = sanitizeString(req.body.packageName || session.packageName, 200);
  if (!loaderGameAllowed(key, packageName)) return denyLoader(res, 403, "GAME_NOT_ALLOWED", "This key is not assigned to this package.", req.requestId);

  session.lastSeenAt = now();
  device.lastSeenAt = now();
  audit(null, "LOADER_LAUNCH_AUTH", "key", key.id, {
    ip: clientIp(req),
    details: { requestId: req.requestId, sessionId: session.id, packageName }
  });
  save();
  ...
  res.json({
    ...
    launch: {
      ...
      message: "Launch authorization approved."
    },
    license: publicLoaderLicense(key),
...
app.post("/api/loader/logout", (req, res) => {
  ...
  if (session) {
    db.loaderSessions = db.loaderSessions.filter(s => s.id !== session.id);
    audit(null, "LOADER_LOGOUT", "key", session.keyId, { ip: clientIp(req), details: { requestId: req.requestId } });
    save();
  }
  res.json({ success: true, requestId: req.requestId, message: "Loader session closed." });
});
...
app.post("/api/owner/loader-settings", auth, requireRoles(ROLE.OWNER), (req, res) => {
  ...
  if (req.body.panelName !== undefined) db.settings.panelName = sanitizeString(req.body.panelName, 100);
  if (req.body.ipGeoLockEnabled !== undefined) db.settings.ipGeoLockEnabled = Boolean(req.body.ipGeoLockEnabled);
  audit(req.user, "UPDATE_LOADER_SETTINGS", "settings", "global", {
    ip: clientIp(req),
    details: { loaderStatus: db.settings.loaderStatus, ipGeoLockEnabled: db.settings.ipGeoLockEnabled }
  });
  save();
  res.json({
    success: true,
...
app.post("/api/settings/loader", auth, requireRoles(ROLE.OWNER), (req, res) => {
  ...
  if (req.body.loaderStatus !== undefined) db.settings.loaderStatus = Boolean(req.body.loaderStatus);
  if (req.body.maintenanceMsg !== undefined) db.settings.maintenanceMsg = sanitizeString(req.body.maintenanceMsg, 1000);
  audit(req.user, "UPDATE_LOADER_SETTINGS", "settings", "global", { ip: clientIp(req), details: { loaderStatus: db.settings.loaderStatus } }); save(); res.json({ success: true, settings: db.settings });
});
...
async function generateKeysPage(){const fallback={"3h":{label:"3 Hour",hours:3,price:10},"1d":{label:"1 Day",hours:24,price:100},"3d":{label:"3 Days",hours:72,price:200},"7d":{label:"7 Days",hours:168,price:350},"15d":{label:"15 Days",hours:360,price:500},"30d":{label:"30 Days",hours:720,price:750},"60d":{label:"60 Days",hours:1440,price:1000}};if(!Object.keys(pricing).length)pricing=fallback;document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Generate Keys</h1><div class="muted">Create license keys using your available balance and permitted duration.</div></div></div><div class="grid2"><div class="card"><h3>Key Generation</h3><p class="muted">Choose duration and quantity. Key timers start after first successful loader verification.</p><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="card"><h3>Manage Keys</h3><p class="muted">View, search and manage existing licenses and statuses.</p><button class="primary" onclick="page(\'manageKeys\')">Manage Keys</button></div></div>';try{const d=await api('/api/pricing');if(d.pricing&&Object.keys(d.pricing).length)pricing=d.pricing}catch(e){}}
...
function generateModal(){const fallback={"3h":{label:"3 Hour",hours:3,price:10},"1d":{label:"1 Day",hours:24,price:100},"3d":{label:"3 Days",hours:72,price:200},"7d":{label:"7 Days",hours:168,price:350},"15d":{label:"15 Days",hours:360,price:500},"30d":{label:"30 Days",hours:720,price:750},"60d":{label:"60 Days",hours:1440,price:1000}};if(!Object.keys(pricing).length)pricing=fallback;let opts=Object.keys(pricing).map(k=>'<option value="'+esc(k)+'">'+esc(pricing[k].label)+' — ₹'+Number(pricing[k].price).toFixed(2)+'</option>').join('');let custom=['owner','admin'].includes(currentUser.role);let price=currentUser.role==='owner'?'<div class="field"><label>Custom Price</label><input id="customPrice" type="number" min="0" step="0.01" value="0"></div>':'';let customKeyField=custom?'<div class="field"><label>Custom Key (Optional)</label><input id="customKey" type="text" value="" placeholder="Optional — e.g. Exl"></div>':'';modal('<h2>Generate Keys</h2><div class="field"><label id="durationHeading">Duration</label><select id="genDuration" onchange="customDurationToggle()">'+opts+(custom?'<option value="custom">Custom Duration</option>':'')+'</select></div>'+customKeyField+'<div id="customFields" class="hidden"><div class="field"><label>Custom Duration (Hours)</label><input id="customHours" type="number" min="1" '+(currentUser.role==='owner'?'':'max="87600"')+' placeholder="Duration in hours"></div>'+price+'<div class="field"><label>Duration Label</label><input id="customLabel" placeholder="e.g. 12 Hours"></div></div><input id="genDevices" type="hidden" value="1"><div class="field"><label>Key Count</label><input id="genCount" type="number" min="1" '+(currentUser.role==='owner'?'':'max="1000"')+' value="1"></div><div id="genErr" class="error hidden"></div><span class="modal-actions"><button class="generate-btn" onclick="generateSave()">Generate</button><button class="action" onclick="closeModal()">Cancel</button></span>')}
function customDurationToggle(){const custom=document.getElementById('genDuration').value==='custom';document.getElementById('customFields').classList.toggle('hidden',!custom);document.getElementById('durationHeading').textContent='Duration';}
...
const SECURITY_SWEEP_MS = 5 * 60 * 1000;
setInterval(() => {
  const before = db.loaderSessions.length;
  cleanupLoaderSessions();
  db.sessions = db.sessions.filter(s => Date.now() < Date.parse(s.expiresAt || 0));
  cleanRateLimits();
  if (db.loaderSessions.length !== before) save();
}, SECURITY_SWEEP_MS).unref();
