...
function createSession(user, req, rememberMe = false, supportSession = false) {
  ...
  const configuredHours = Number(db.settings?.sessionTimeoutHours || 24);
  const inactivityMs = Math.max(24, Math.min(168, configuredHours)) * 60 * 60 * 1000;
  const maxAge = rememberMe ? 30 * 24 * 60 * 60 * 1000 : inactivityMs;
...
function getSession(req) {
  ...
  if (session.rememberMe) {
    ...
    session.expiresAt = new Date(Date.now() + inactivityMs).toISOString();
  }
  save();
...
const PAGE_PATHS={dashboard:'/Dashboard',generateKeys:'/Generate-Keys',manageKeys:'/Manage-Keys',users:'/Users',referrals:'/Referrals',transactions:'/Transactions',activity:'/Activity',audit:'/Audit-Logs',settings:'/Settings'};
...
async function dashboard(){try{const d=await api('/api/dashboard');const s=d.stats||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Dashboard</h1><div class="muted">License and account overview</div></div><button class="generate-btn" onclick="generateModal()">Generate Key</button></div><div class="grid">'+stat('Active Keys',s.activeKeys||0)+stat('Expired Keys',s.expiredKeys||0)+stat('Blocked Keys',s.blockedKeys||0)+stat(currentUser.role==='owner'?'Users':'My Keys',s.users||s.activeKeys||0)+'</div><div class="grid2" style="margin-top:14px">'+stat('Balance','₹'+Number(s.balance||0).toFixed(2))+stat('Resellers',s.resellers||0)+'</div><div class="card" style="margin-top:14px"><h3>Loader Status</h3><p><span class="badge '+(d.settings?.loaderStatus?'ok':'bad')+'">'+(d.settings?.loaderStatus?'ONLINE':'MAINTENANCE')+'</span></p><p class="muted">'+esc(d.settings?.loaderStatus?'Loader verification is enabled.':(d.settings?.maintenanceMsg||'Loader disabled.'))+'</p></div><div class="card" style="margin-top:14px"><h3>Account Creation</h3><p class="muted">Exact account creation date and time</p><b>'+esc(fmt(currentUser.createdAt))+'</b></div>'}catch(e){toast(e.message)}}
async function generateKeysPage(){try{pricing=(await api('/api/pricing')).pricing||{};document.getElementById('content').innerHTML=consumeFlash()+keySuccessCard(window.__generatedKeys)+'<div class="page-head"><div><h1>Generate Keys</h1><div class="muted">Create license keys using your available balance and permitted duration.</div></div><button class="manage-btn" onclick="generateModal()">Generate Keys</button></div><div class="grid2"><div class="card"><h3>Key Generation</h3><p class="muted">Choose duration, device limit and quantity. Key timers start after first successful loader verification.</p><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="card"><h3>Manage Keys</h3><p class="muted">View, search and manage existing licenses, device bindings and statuses.</p><button class="manage-btn" onclick="page(\'manageKeys\')">Manage Keys</button></div></div>'}catch(e){setFlash('Generate Keys load failed: '+e.message,'error');renderFlashOnly()}}
...
function userActions(u){const can=u.id!==currentUser.id;return '<div class="actions">'+(can?'<button class="action" onclick="enterPanel(\''+esc(u.id)+'\')">Enter Panel</button>':'')+(can?'<button class="action" onclick="adjustBalance(\''+esc(u.id)+'\')">Balance</button>':'')+(can&&u.status==='active'?'<button class="block-btn" onclick="toggleUserStatus(\''+esc(u.id)+'\')">Block</button>':'')+(can&&u.status==='blocked'?'<button class="unblock-btn" onclick="toggleUserStatus(\''+esc(u.id)+'\')">Unblock</button>':'')+(can?'<button class="danger" onclick="deleteUser(\''+esc(u.id)+'\')">Delete</button>':'')+'</div>'}
