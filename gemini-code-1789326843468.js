/*
 * Lexi Loader - Reseller Panel Backend & Web UI
 * Fully restored with all previous features, pricing editor, audit logs, file manifests,
 * glowing neon gaming UI, emojis, and clean branding (no version numbers).
 */
...
app.post("/api/referrals/create", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  let role = normalizeRole(req.body.role); if (target.role === ROLE.ADMIN) role = ROLE.RESELLER;
...
app.delete("/api/users/:id", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  const target = findUserById(req.params.id); if (!target || !canManageUser(req.user, target)) return res.status(404).json({ success: false, message: "Not found." });
  if (target.role === ROLE.OWNER) return res.status(403).json({ success: false, message: "Denied." });
  target.status = STATUS.DELETED; db.sessions = db.sessions.filter(x => x.userId !== target.id); audit(req.user, "DELETE_USER", "user", target.id, { ip: clientIp(req) }); save();
  res.json({ success: true, message: "Deleted." });
});
...
function generateLicenseKey(prefix = "Lexi", duration = "", durationLabel = "") {
  const cleanPrefix = String(prefix || "Lexi").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 20) || "Lexi";
...
app.post("/api/keys/:id/update", auth, (req, res) => {
  ...
  key.key = nextKey; key.deviceLimit = deviceLimit; key.gamePackage = sanitizeString(req.body.gamePackage ?? key.gamePackage ?? "", 200); key.status = status;
  audit(req.user, "UPDATE_KEY", "key", key.id, { ip: clientIp(req) }); save();
...
app.get("/api/loader/version", (req, res) => loaderSuccess(res, { version: { currentVersion: DEFAULT_LOADER_VERSION } }));
...
/* ---------- Dashboard, Activity, Settings, Pricing ---------- */
...
app.post("/api/pricing/update", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const incoming = req.body.pricing; if (!incoming || typeof incoming !== "object") return res.status(400).json({ success: false, message: "Invalid." });
  const next = {};
  for (const key of Object.keys(PRICING)) {
    const row = incoming[key]; if (!row) return res.status(400).json({ success: false, message: `Missing ${key}.` });
    next[key] = { label: sanitizeString(row.label || key, 60), hours: Number(row.hours), price: Number(row.price) };
  }
  db.pricing = next; audit(req.user, "UPDATE_PRICING", "pricing", "global", { ip: clientIp(req), details: next }); save();
...
/* ---------- Web panel HTML with Glowing Neon Gaming Theme & All Features ---------- */
...
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';let items=navItem('dashboard','Dashboard','📊')+navItem('generateKeys','Generate Keys','🔑')+navItem('manageKeys','Manage Keys','🛡️');if(owner)items+=navItem('users','Users','👥');if(owner||admin)items+=navItem('referrals','Referrals','🎟️')+navItem('transactions','Transactions','💰')+navItem('activity','Activity','⚡');if(owner)items+=navItem('loader','Loader Control','⚙️')+navItem('pricing','Pricing Management','💎');items+=navItem('settings','Settings','🔧');return '<div id="sideOverlay" class="side-overlay" onclick="closeSideMenu()"></div><aside id="sidebar" class="sidebar" onclick="event.stopPropagation()"><div class="brand">⚡ Lexi <span>Loader</span></div><div class="side-user"><b>👤 '+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">⭐ '+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">💰 Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+items+'</div><div class="side-bottom"><button class="action full" onclick="logout()">🚪 Logout</button></div></aside>'}
...
async function page(name,opts={}){currentPage=name;closeSideMenu();document.querySelectorAll('.navbtn').forEach(b=>b.classList.toggle('active',b.dataset.page===name));if(!currentUser)return;if(name==='dashboard')return dashboard();if(name==='generateKeys')return generateKeysPage();if(name==='manageKeys')return keysPage();if(name==='users'&&currentUser.role==='owner')return usersPage();if(name==='referrals')return referralsPage();if(name==='transactions')return transactionsPage();if(name==='activity')return activityPage();if(name==='loader'&&currentUser.role==='owner')return loaderPage();if(name==='pricing'&&currentUser.role==='owner')return pricingPage();if(name==='settings')return settingsPage();return dashboard()}
...
async function pricingPage(){try{const p=await api('/api/pricing');pricing=p.pricing||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>💎 Pricing Management</h1><div class="muted">Customize duration pricing</div></div></div><div class="card"><div id="priceEditor">'+Object.keys(pricing).map(k=>'<div class="setting-row"><div><b>'+esc(pricing[k].label)+'</b><small>'+pricing[k].hours+' hours</small></div><input id="price_'+esc(k)+'" type="number" value="'+Number(pricing[k].price)+'" style="max-width:140px"></div>').join('')+'</div><button class="primary" style="margin-top:16px" onclick="savePricing()">💾 Save Pricing</button></div>'}catch(e){toast(e.message)}}
async function savePricing(){try{const out={};for(const k of Object.keys(pricing))out[k]={label:pricing[k].label,hours:pricing[k].hours,price:Number(document.getElementById('price_'+k).value)};await api('/api/pricing/update',{method:'POST',body:JSON.stringify({pricing:out})});toast('Pricing saved successfully!')}catch(e){toast(e.message)}}
...
app.get(/^\/(Dashboard|Generate-Keys|Manage-Keys|Users|Referrals|Transactions|Activity|Settings|Loader|Pricing)(?:\/.*)?$/i, (req, res) => res.type("html").send(PANEL_HTML));
