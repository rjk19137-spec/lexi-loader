 ...
 * Merged backend with full loader/session management, audit logs, file manifests, and neon panel features.
 */
...
const LOADER_MAX_BODY = 32 * 1024;
const LOADER_API_VERSION = 1;
const DEFAULT_LOADER_VERSION = "1.0.0";
...
function migrateDatabase() {
  ...
  if (db.settings.loaderApiVersion === undefined) db.settings.loaderApiVersion = LOADER_API_VERSION;
  if (db.settings.loaderVersion === undefined) db.settings.loaderVersion = DEFAULT_LOADER_VERSION;
  if (db.settings.loaderMinimumVersion === undefined) db.settings.loaderMinimumVersion = DEFAULT_LOADER_VERSION;
  if (db.settings.loaderUpdateUrl === undefined) db.settings.loaderUpdateUrl = "";
  if (db.settings.loaderUpdateSha256 === undefined) db.settings.loaderUpdateSha256 = "";
  if (db.settings.loaderUpdateSize === undefined) db.settings.loaderUpdateSize = 0;
  if (db.settings.loaderUpdateRequired === undefined) db.settings.loaderUpdateRequired = false;
  if (db.settings.loaderUpdateNotes === undefined) db.settings.loaderUpdateNotes = "";
  if (!db.settings.loaderFeatures || typeof db.settings.loaderFeatures !== "object") db.settings.loaderFeatures = { BT: true, AIM: true, ESP: true, IPAD_VIEW: true };
  if (!Array.isArray(db.settings.loaderFileManifests)) db.settings.loaderFileManifests = [];
...
function resetWindow(user) {
  if (!user.resetWindowStartedAt || Date.now() - Date.parse(user.resetWindowStartedAt) >= RESET_WINDOW_MS) {
    user.resetAttempts = 0; user.resetWindowStartedAt = now();
  }
}
function validUsername(username) { return /^[a-zA-Z0-9_.-]{3,32}$/.test(username); }
...
function generateLicenseKey(prefix = "Lexi-Loader", duration = "", durationLabel = "") {
  const cleanPrefix = String(prefix || "Lexi-Loader").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 20) || "Lexi-Loader";
...
function ensureUniqueKey(prefix, duration, durationLabel) {
  ...
  throw new Error("Unable to generate a unique license key after 100 attempts.");
}
...
/* ---------- APIs ---------- */
app.get("/api/health", (req, res) => { res.json({ success: true, service: "Lexi Loader Backend", version: db.version, time: now() }); });
...
app.post("/api/referrals/create", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  if (req.user.role !== ROLE.OWNER && (!Number.isFinite(startingBalance) || startingBalance < 3000 || startingBalance > 50000)) return res.status(400).json({ success: false, message: "Invalid balance." });
  ...
  db.referrals.push(referral); audit(req.user, "CREATE_REFERRAL", "referral", referral.id, { ip: clientIp(req), details: { code, targetId: target.id, role, startingBalance, usageLimit: 0, expiresAt } }); save();
...
app.get("/api/referrals", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  if (requestedUserId) { const target = findUserById(requestedUserId); if (!target || !canViewUser(req.user, target)) return res.status(404).json({ success: false, message: "Not found." }); }
  const rows = db.referrals.filter(r => requestedUserId ? r.creatorId === requestedUserId : (req.user.role === ROLE.OWNER ? true : r.creatorId === req.user.id));
...
app.post("/api/referrals/:id/balance", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  if (mode === "add") after = before + amount; else if (mode === "subtract") after = Math.max(0, before - amount); else if (mode !== "set") return res.status(400).json({ success:false, message:"Invalid mode." });
...
app.post("/api/users/:id/balance", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  ...
  if (mode === "add") after = before + amount; else if (mode === "subtract") { after = before - amount; if (after < 0) return res.status(400).json({ success: false, message: "Negative." }); } else if (mode === "set") after = amount; else return res.status(400).json({ success: false, message: "Invalid mode." });
...
app.delete("/api/users/:id", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN), (req, res) => {
  const target = findUserById(req.params.id); if (!target || !canManageUser(req.user, target)) return res.status(404).json({ success: false, message: "Not found." });
  if (target.role === ROLE.OWNER) return res.status(403).json({ success: false, message: "Denied." });
  target.status = STATUS.DELETED; db.sessions = db.sessions.filter(x => x.userId !== target.id); audit(req.user, "DELETE_USER", "user", target.id, { ip: clientIp(req), details: { username: target.username } }); save();
  res.json({ success: true, message: "Deleted." });
});
...
app.post("/api/keys/generate", auth, (req, res) => {
  ...
  if (customKey && quantity !== 1) return res.status(400).json({ success: false, message: "Custom key can generate 1 key." });
  if (customKey && !/^[a-zA-Z0-9_-]{1,100}$/.test(customKey)) return res.status(400).json({ success: false, message: "Invalid custom key format." });
  const customPrice = req.body.customPrice === undefined || req.body.customPrice === null ? null : Number(req.body.customPrice);
  if (!db.pricing[duration] && duration !== "custom" && ![ROLE.OWNER, ROLE.ADMIN].includes(req.user.role)) return res.status(400).json({ success: false, message: "Invalid duration." });
  if (!Number.isInteger(deviceLimit) || deviceLimit < 1 || (req.user.role !== ROLE.OWNER && deviceLimit > MAX_DEVICE_LIMIT)) return res.status(400).json({ success: false, message: "Invalid device limit." });
  if (!Number.isInteger(quantity) || quantity < 1 || (req.user.role !== ROLE.OWNER && quantity > MAX_KEY_QUANTITY)) return res.status(400).json({ success: false, message: "Invalid count." });
  ...
  if (db.pricing[duration]) { 
    ...
    if (![ROLE.OWNER, ROLE.ADMIN].includes(req.user.role) && duration !== "custom") return res.status(403).json({ success: false, message: "Denied." });
    ...
    if (!Number.isFinite(durationHours) || durationHours <= 0 || (req.user.role !== ROLE.OWNER && durationHours > 87600)) return res.status(400).json({ success: false, message: "Invalid duration." });
    if (!Number.isFinite(basePrice) || basePrice < 0) return res.status(400).json({ success: false, message: "Invalid price." });
  }
  if (req.user.role === ROLE.ADMIN && customPrice !== null) return res.status(400).json({ success: false, message: "Denied." });
  const totalPrice = Number((basePrice * quantity).toFixed(2)); const beforeBalance = Number(req.user.balance || 0);
  if (req.user.role !== ROLE.OWNER && beforeBalance < totalPrice) return res.status(402).json({ success: false, code: "INSUFFICIENT_BALANCE", message: "Insufficient balance.", required: totalPrice, available: beforeBalance, shortfall: Number((totalPrice - beforeBalance).toFixed(2)) });

  const batchId = randomId("batch"); const createdAt = Date.now(); const generated = [];
  for (let i = 0; i < quantity; i++) {
    const key = { id: randomId("key"), key: customKey || ensureUniqueKey("LexiLoader", duration, durationLabel), creatorId: req.user.id, batchId, duration, durationHours, durationLabel, basePrice, deviceLimit, gamePackage, status: KEY_STATUS.ACTIVE, createdAt: new Date(createdAt).toISOString(), startedAt: null, expiryAt: null, active_hack: "ESP", hack_expiry: null, ipLock: db.settings.ipGeoLockEnabled && req.body.ipLock ? sanitizeString(req.body.ipLock, 100) : null, geoLock: db.settings.ipGeoLockEnabled && req.body.geoLock ? sanitizeString(req.body.geoLock, 100) : null, deletedAt: null };
  ...
  if (req.user.role !== ROLE.OWNER && totalPrice > 0) {
    ...
    transaction(req.user, "KEY_GENERATION", -totalPrice, beforeBalance, afterBalance, req.user.id, { batchId, quantity, duration, deviceLimit, unitPrice: basePrice });
  }
  audit(req.user, "GENERATE_KEYS", "batch", batchId, { ip: clientIp(req), details: { quantity, duration, deviceLimit, totalPrice, ownerUnlimited: req.user.role === ROLE.OWNER } }); save();
  res.status(201).json({ success: true, message: `${quantity} key(s) generated.`, batchId, pricing: { unitPrice: basePrice, quantity, totalPrice, balanceBefore: beforeBalance, balanceAfter: req.user.role === ROLE.OWNER ? beforeBalance : req.user.balance }, keys: generated });
});
app.post("/api/keys/:id/update", auth, (req, res) => {
  const key = db.keys.find(k => k.id === req.params.id); if (!key || !canManageKey(req.user, key)) return res.status(404).json({ success:false, message:"Not found." });
  const nextKey = sanitizeString(req.body.key || key.key, 100).trim(); if (!nextKey || !/^[A-Za-z0-9_-]+$/.test(nextKey)) return res.status(400).json({success:false,message:"Invalid format."});
  if (db.keys.some(k => k.id !== key.id && k.key === nextKey)) return res.status(409).json({success:false,message:"Exists."});
  const deviceLimit = Number(req.body.deviceLimit ?? key.deviceLimit); if (!Number.isInteger(deviceLimit) || deviceLimit < 1 || deviceLimit > 9999) return res.status(400).json({success:false,message:"Invalid limit."});
  const status = String(req.body.status || key.status || "active").toLowerCase(); if (![KEY_STATUS.ACTIVE, KEY_STATUS.BLOCKED, KEY_STATUS.DELETED].includes(status)) return res.status(400).json({success:false,message:"Invalid status."});
  ...
  res.json({success:true,message:"Updated.",key:publicKey(key)});
});
app.post("/api/keys/:id/reset", auth, (req, res) => {
  const key = db.keys.find(k => k.id === req.params.id); if (!key || !canManageKey(req.user, key)) return res.status(404).json({ success: false, message: "Not found." });
  const before = db.devices.length; db.devices = db.devices.filter(d => d.keyId !== key.id); const removed = before - db.devices.length;
  audit(req.user, "RESET_KEY_DEVICES", "key", key.id, { ip: clientIp(req), details: { removedDevices: removed } }); save();
  res.json({ success: true, message: "Reset.", removedDevices: removed, key: publicKey(key) });
});
app.post("/api/keys/:id/status", auth, (req, res) => {
  const key = db.keys.find(k => k.id === req.params.id); if (!key || !canManageKey(req.user, key)) return res.status(404).json({ success: false, message: "Not found." });
  const status = String(req.body.status || "").toLowerCase(); if (![KEY_STATUS.ACTIVE, KEY_STATUS.BLOCKED, KEY_STATUS.DELETED].includes(status)) return res.status(400).json({ success: false, message: "Invalid." });
  if (status === KEY_STATUS.ACTIVE && keyStatus(key) === KEY_STATUS.EXPIRED) return res.status(400).json({ success: false, message: "Expired." });
...
function loaderPackageForKey(key) { return sanitizeString(key?.gamePackage || db.settings.loaderTargetPackage || DEFAULT_TARGET_PACKAGE, 200); }
function loaderVersionPolicy(version) {
  const current = normalizeVersion(db.settings.loaderVersion || DEFAULT_LOADER_VERSION);
  const minimum = normalizeVersion(db.settings.loaderMinimumVersion || current);
  const supplied = normalizeVersion(version);
  return { current, minimum, supplied, compatible: compareVersions(supplied, minimum) >= 0, currentMatch: compareVersions(supplied, current) === 0 };
}
function loaderPublicConfig() {
  const s = db.settings || {};
  return {
    apiVersion: Number(s.loaderApiVersion || LOADER_API_VERSION),
    enabled: Boolean(s.loaderStatus) && s.systemOnline !== false,
    systemOnline: s.systemOnline !== false,
    maintenanceMessage: sanitizeString(s.maintenanceMsg || "Loader is temporarily under maintenance.", 1000),
    panelName: sanitizeString(s.panelName || "Lexi Loader", 100),
    targetPackage: sanitizeString(s.loaderTargetPackage || DEFAULT_TARGET_PACKAGE, 200),
    appNoticeEnabled: Boolean(s.appNoticeEnabled),
    appNotice: sanitizeString(s.appNotice || "", 2000),
    appNoticeType: sanitizeString(s.appNoticeType || "info", 30),
    features: { BT: Boolean(s.loaderFeatures?.BT), AIM: Boolean(s.loaderFeatures?.AIM), ESP: Boolean(s.loaderFeatures?.ESP), IPAD_VIEW: Boolean(s.loaderFeatures?.IPAD_VIEW) }
  };
}
function loaderVersionPayload() {
  const s = db.settings || {};
  return {
    apiVersion: Number(s.loaderApiVersion || LOADER_API_VERSION),
    currentVersion: normalizeVersion(s.loaderVersion || DEFAULT_LOADER_VERSION),
    minimumVersion: normalizeVersion(s.loaderMinimumVersion || s.loaderVersion || DEFAULT_LOADER_VERSION),
    updateRequired: Boolean(s.loaderUpdateRequired),
    updateUrl: sanitizeString(s.loaderUpdateUrl || "", 2000),
    updateSha256: String(s.loaderUpdateSha256 || "").trim().toLowerCase(),
    updateSize: Math.max(0, Number(s.loaderUpdateSize || 0)),
    updateNotes: sanitizeString(s.loaderUpdateNotes || "", 4000)
  };
}
function cleanupLoaderSessions() {
  const cutoff = Date.now();
  db.loaderSessions = (db.loaderSessions || []).filter(s => Date.parse(s.expiresAt || 0) > cutoff);
}
function createLoaderSession({ key, device, hwid, packageName, loaderVersion, ip }) {
  cleanupLoaderSessions();
  const token = randomToken(32);
  const createdAt = Date.now();
  db.loaderSessions.push({
    id: randomId("lsess"), tokenHash: hashLoaderToken(token), keyId: key.id, deviceId: device.id,
    hwid, packageName, loaderVersion: normalizeVersion(loaderVersion), ip,
    createdAt: new Date(createdAt).toISOString(), lastSeenAt: new Date(createdAt).toISOString(),
    expiresAt: new Date(createdAt + LOADER_SESSION_TTL_MS).toISOString()
  });
  return { token, expiresAt: new Date(createdAt + LOADER_SESSION_TTL_MS).toISOString() };
}
function getLoaderToken(req) {
  const authz = String(req.headers.authorization || "");
  if (!authz.toLowerCase().startsWith("bearer ")) return "";
  return authz.slice(7).trim();
}
function requireLoaderSession(req, res) {
  cleanupLoaderSessions();
  const token = getLoaderToken(req);
  if (!token) { loaderResponse(res, 401, "SESSION_INVALID", "Authorization Failed"); return null; }
  const tokenHash = hashLoaderToken(token);
  const session = db.loaderSessions.find(x => safeEqual(x.tokenHash, tokenHash));
  if (!session) { loaderResponse(res, 401, "SESSION_INVALID", "Authorization Failed"); return null; }
  if (Date.now() >= Date.parse(session.expiresAt || 0)) { db.loaderSessions = db.loaderSessions.filter(x => x.id !== session.id); save(); loaderResponse(res, 401, "SESSION_EXPIRED", "Session Expired"); return null; }
  if (session.ip && session.ip !== clientIp(req)) { loaderResponse(res, 403, "AUTHORIZATION_REVOKED", "Authorization Revoked"); return null; }
  session.lastSeenAt = now();
  req.loaderSession = session;
  req.loaderTokenHash = tokenHash;
  return session;
}
function findLoaderKey(session) { return db.keys.find(k => k.id === session.keyId); }
function checkLoaderAuthorization(session, packageName, loaderVersion, req) {
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return { status: 503, code: "LOADER_DISABLED", message: db.settings.maintenanceMsg || "Loader is temporarily under maintenance." };
  const key = findLoaderKey(session);
  if (!key || key.status === KEY_STATUS.DELETED) return { status: 404, code: "KEY_NOT_FOUND", message: "Invalid Key" };
  const status = keyStatus(key);
  if (status === KEY_STATUS.BLOCKED) return { status: 403, code: "KEY_BLOCKED", message: "Key Blocked" };
  if (status === KEY_STATUS.EXPIRED) return { status: 403, code: "KEY_EXPIRED", message: "Expired Key" };
  const policy = loaderVersionPolicy(loaderVersion);
  if (!policy.compatible) return { status: 426, code: "VERSION_UNSUPPORTED", message: "Please Update Loader", version: loaderVersionPayload() };
  const allowedPackage = loaderPackageForKey(key);
  if (allowedPackage && packageName !== allowedPackage) return { status: 403, code: "GAME_NOT_ALLOWED", message: "Application Not Allowed" };
  if (key.ipLock && key.ipLock !== clientIp(req)) return { status: 403, code: "IP_LOCKED", message: "IP Locked" };
  if (key.geoLock) { const country = String(req.headers["x-country-code"] || "").toUpperCase(); if (!country || country !== String(key.geoLock).toUpperCase()) return { status: 403, code: "GEO_LOCKED", message: "Geo Locked" }; }
  const device = db.devices.find(d => d.id === session.deviceId && d.keyId === key.id);
  if (!device || !safeEqual(device.hwid, session.hwid)) return { status: 403, code: "AUTHORIZATION_REVOKED", message: "Authorization Revoked" };
  const liveDevices = db.devices.filter(d => d.keyId === key.id);
  if (liveDevices.length > Number(key.deviceLimit || 1)) return { status: 403, code: "DEVICE_LIMIT_REACHED", message: "Device Limit Full" };
  return { key, device, policy };
}
function loaderLicenseSnapshot(key) {
  return { key: key.key, startedAt: key.startedAt || null, expiryAt: key.expiryAt || null, durationHours: Number(key.durationHours || 0), deviceLimit: Number(key.deviceLimit || 1), devicesUsed: db.devices.filter(d => d.keyId === key.id).length };
}

app.get("/api/loader/config", (req, res) => loaderSuccess(res, { loader: loaderPublicConfig(), pricing: db.pricing }));
app.get("/api/loader/version", (req, res) => loaderSuccess(res, { version: loaderVersionPayload() }));
app.get("/api/loader/manifest", (req, res) => loaderSuccess(res, { targetPackage: db.settings.loaderTargetPackage || DEFAULT_TARGET_PACKAGE, manifests: Array.isArray(db.settings.loaderFileManifests) ? db.settings.loaderFileManifests : [] }));

// Added  sync-mode endpoint for PostLaunchManager server synchronization
app.get("/api/loader/sync-mode", (req, res) => {
  const mode = sanitizeString(req.query.mode || "AIM", 50);
  return res.json({
    success: true,
    mode: mode,
    loaderVersion: db.settings.loaderVersion || "V-1.0",
    expiryText: "Valid Till  = 12-00-00   31-12-2026",
    time: now()
  });
});
...
app.post("/api/loader/verify", (req, res) => {
  ...
  const packageName = sanitizeString(req.body.packageName, 200);
  const loaderVersion = normalizeVersion(req.body.loaderVersion);
  const ip = clientIp(req);
  if (!db.settings.loaderStatus || db.settings.systemOnline === false) return loaderResponse(res, 503, "LOADER_DISABLED", db.settings.maintenanceMsg || "Loader is temporarily under maintenance.");
  if (!licenseKey || !hwid || !deviceName || !packageName || !loaderVersion) return loaderResponse(res, 400, "INVALID_REQUEST", "Required loader information is missing.");
  const policy = loaderVersionPolicy(loaderVersion);
  if (!policy.compatible) return loaderResponse(res, 426, "VERSION_UNSUPPORTED", "Please Update Loader", { version: loaderVersionPayload() });
  const key = db.keys.find(k => k.key === licenseKey);
  if (!key || key.status === KEY_STATUS.DELETED) return loaderResponse(res, 404, "KEY_NOT_FOUND", "Invalid Key");
  const status = keyStatus(key);
  if (status === KEY_STATUS.BLOCKED) return loaderResponse(res, 403, "KEY_BLOCKED", "Key Blocked");
  if (status === KEY_STATUS.EXPIRED) return loaderResponse(res, 403, "KEY_EXPIRED", "Expired Key");
  const allowedPackage = loaderPackageForKey(key);
  if (allowedPackage && packageName !== allowedPackage) return loaderResponse(res, 403, "GAME_NOT_ALLOWED", "Application Not Allowed");
  if (key.ipLock && key.ipLock !== ip) return loaderResponse(res, 403, "IP_LOCKED", "IP Locked");
  if (key.geoLock) { const country = String(req.headers["x-country-code"] || "").toUpperCase(); if (!country || country !== String(key.geoLock).toUpperCase()) return loaderResponse(res, 403, "GEO_LOCKED", "Geo Locked"); }
  ...
  if (!device) {
    if (devices.length >= Number(key.deviceLimit || 1)) return loaderResponse(res, 403, "DEVICE_LIMIT_REACHED", "Device Limit Full");
    if (firstActivation) { const startedMs = Date.now(); key.startedAt = new Date(startedMs).toISOString(); key.expiryAt = new Date(startedMs + Number(key.durationHours || 0) * 60 * 60 * 1000).toISOString(); }
    device = { id: randomId("dev"), keyId: key.id, hwid, deviceName, ip, firstSeenAt: now(), lastSeenAt: now() };
    db.devices.push(device);
  } else {
    if (firstActivation) { const startedMs = Date.now(); key.startedAt = new Date(startedMs).toISOString(); key.expiryAt = new Date(startedMs + Number(key.durationHours || 0) * 60 * 60 * 1000).toISOString(); }
    device.lastSeenAt = now(); device.ip = ip; if (deviceName) device.deviceName = deviceName;
  }
  const session = createLoaderSession({ key, device, hwid, packageName, loaderVersion, ip });
  audit(null, "LOADER_VERIFY", "key", key.id, { ip, details: { requestId: req.requestId, deviceId: device.id, packageName, loaderVersion } });
  save();
  return loaderSuccess(res, { code: "VERIFIED", token: session.token, session: { expiresAt: session.expiresAt, ttlSeconds: Math.floor(LOADER_SESSION_TTL_MS / 1000) }, license: loaderLicenseSnapshot(key), loader: loaderPublicConfig(), version: loaderVersionPayload() });
});


app.post("/api/loader/feature_action", (req, res) => {
  const hwid = sanitizeString(req.body.hwid, 300);
  const selectedFeature = sanitizeString(req.body.selected_feature, 50).toUpperCase();
  ...
  if (!hwid) return res.status(400).json({ success: false, reason: "HWID is required." });

  const device = db.devices.find(d => safeEqual(d.hwid, hwid));
  if (!device) {
    return res.status(403).json({ success: false, reason: "Device authorization not found." });
  }
...
app.post("/api/loader/launch", (req, res) => {
  const session = requireLoaderSession(req, res); if (!session) return;
  const packageName = sanitizeString(req.body.packageName || session.packageName, 200);
  const loaderVersion = normalizeVersion(req.body.loaderVersion || session.loaderVersion);
  const authz = checkLoaderAuthorization(session, packageName, loaderVersion, req);
  if (authz.status) { audit(null, "LOADER_LAUNCH_AUTH", "session", session.id, { ip: clientIp(req), details: { requestId: req.requestId, code: authz.code, packageName, loaderVersion } }); save(); return loaderResponse(res, authz.status, authz.code, authz.message, authz.version ? { version: authz.version } : {}); }
  const key = authz.key;
  session.lastSeenAt = now();
  audit(null, "LOADER_LAUNCH_AUTH", "key", key.id, { ip: clientIp(req), details: { requestId: req.requestId, result: "AUTHORIZED", packageName, loaderVersion } });
  save();
  return loaderSuccess(res, { code: "LAUNCH_AUTHORIZED", authorized: true, packageName, hack: key.active_hack || "ESP", hack_expiry: key.hack_expiry || null, license: loaderLicenseSnapshot(key), loader: loaderPublicConfig(), features: loaderPublicConfig().features });
});

app.post("/api/loader/logout", (req, res) => {
  const token = getLoaderToken(req);
  if (!token) return loaderSuccess(res, { code: "NO_SESSION" });
  const hash = hashLoaderToken(token);
  const before = db.loaderSessions.length;
  const session = db.loaderSessions.find(s => safeEqual(s.tokenHash, hash));
  db.loaderSessions = db.loaderSessions.filter(s => !safeEqual(s.tokenHash, hash));
  if (session) audit(null, "LOADER_LOGOUT", "session", session.id, { ip: clientIp(req), details: { requestId: req.requestId } });
  if (before !== db.loaderSessions.length) save();
  return loaderSuccess(res, { code: "LOGGED_OUT" });
});

app.post("/api/loader/heartbeat", (req, res) => {
  const session = requireLoaderSession(req, res); if (!session) return;
  const authz = checkLoaderAuthorization(session, session.packageName, session.loaderVersion, req);
  if (authz.status) return loaderResponse(res, authz.status, authz.code, authz.message, authz.version ? { version: authz.version } : {});
  session.lastSeenAt = now();
  save();
  return loaderSuccess(res, { code: "SESSION_ACTIVE", session: { expiresAt: session.expiresAt } });
});
...
app.post("/api/owner/loader-settings", auth, requireRoles(ROLE.OWNER), (req, res) => {
  ...
  if (b.panelName !== undefined) db.settings.panelName = sanitizeString(b.panelName, 100);
  if (b.ipGeoLockEnabled !== undefined) db.settings.ipGeoLockEnabled = Boolean(b.ipGeoLockEnabled);
  if (b.loaderVersion !== undefined) db.settings.loaderVersion = normalizeVersion(b.loaderVersion);
  if (b.loaderMinimumVersion !== undefined) db.settings.loaderMinimumVersion = normalizeVersion(b.loaderMinimumVersion);
  if (b.loaderTargetPackage !== undefined) db.settings.loaderTargetPackage = sanitizeString(b.loaderTargetPackage, 200);
  if (b.loaderUpdateUrl !== undefined) db.settings.loaderUpdateUrl = sanitizeString(b.loaderUpdateUrl, 2000);
  if (b.loaderUpdateSha256 !== undefined) db.settings.loaderUpdateSha256 = String(b.loaderUpdateSha256 || "").trim().toLowerCase().slice(0, 128);
  if (b.loaderUpdateSize !== undefined) db.settings.loaderUpdateSize = Math.max(0, Number(b.loaderUpdateSize) || 0);
  if (b.loaderUpdateRequired !== undefined) db.settings.loaderUpdateRequired = Boolean(b.loaderUpdateRequired);
  if (b.loaderUpdateNotes !== undefined) db.settings.loaderUpdateNotes = sanitizeString(b.loaderUpdateNotes, 4000);
  if (b.loaderFeatures && typeof b.loaderFeatures === "object" && !Array.isArray(b.loaderFeatures)) db.settings.loaderFeatures = { ...db.settings.loaderFeatures, BT: Boolean(b.loaderFeatures.BT), AIM: Boolean(b.loaderFeatures.AIM), ESP: Boolean(b.loaderFeatures.ESP), IPAD_VIEW: Boolean(b.loaderFeatures.IPAD_VIEW) };
  if (Array.isArray(b.loaderFileManifests)) db.settings.loaderFileManifests = b.loaderFileManifests.slice(0, 100).map(x => ({ id: sanitizeString(x.id, 100), version: sanitizeString(x.version, 50), size: Math.max(0, Number(x.size) || 0), sha256: String(x.sha256 || "").trim().toLowerCase().slice(0, 128), required: Boolean(x.required), pathClass: sanitizeString(x.pathClass || "loader-owned", 50) }));
  audit(req.user, "UPDATE_LOADER_SETTINGS", "settings", "global", { ip: clientIp(req), details: { requestId: req.requestId, loaderVersion: db.settings.loaderVersion, minimumVersion: db.settings.loaderMinimumVersion, targetPackage: db.settings.loaderTargetPackage } });
  save();
  return res.json({ success: true, requestId: req.requestId, settings: loaderPublicConfig(), version: loaderVersionPayload(), manifests: db.settings.loaderFileManifests });
});
...
app.get("/api/dashboard", auth, (req, res) => {
  ...
  const counts = { activeKeys: visibleKeys.filter(k => keyStatus(k) === KEY_STATUS.ACTIVE).length, expiredKeys: visibleKeys.filter(k => keyStatus(k) === KEY_STATUS.EXPIRED).length, blockedKeys: visibleKeys.filter(k => keyStatus(k) === KEY_STATUS.BLOCKED).length, deletedKeys: visibleKeys.filter(k => keyStatus(k) === KEY_STATUS.DELETED).length, users: visibleUsers.length, admins: visibleUsers.filter(u => u.role === ROLE.ADMIN).length, resellers: visibleUsers.filter(u => u.role === ROLE.RESELLER).length };
  const ownTransactions = db.transactions.filter(t => req.user.role === ROLE.OWNER ? true : t.actorId === req.user.id || t.targetUserId === req.user.id );
  res.json({ success: true, user: publicUser(req.user), stats: { ...counts, balance: Number(req.user.balance || 0), totalVisibleBalance: visibleUsers.reduce((sum, u) => sum + Number(u.balance || 0), 0), referrals: db.referrals.filter(r => req.user.role === ROLE.OWNER || r.creatorId === req.user.id).length, transactions: ownTransactions.length }, settings: { loaderStatus: db.settings.loaderStatus, maintenanceMsg: db.settings.maintenanceMsg, panelName: db.settings.panelName, ipGeoLockEnabled: db.settings.ipGeoLockEnabled } });
});
app.get("/api/activity", auth, (req, res) => {
  let rows; if (req.user.role === ROLE.OWNER) { rows = [...db.auditLogs]; } else { const visible = new Set(db.users.filter(u => u.id === req.user.id || (u.parentId === req.user.id && u.role === ROLE.RESELLER)).map(u => u.id)); rows = db.auditLogs.filter(x => visible.has(x.actorId) || visible.has(x.targetId)); }
  res.json({ success: true, logs: rows.reverse().slice(0, 1000) });
});
app.get("/api/audit-logs", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const q = sanitizeString(req.query.search, 100).toLowerCase(); let rows = [...db.auditLogs].reverse();
  if (q) rows = rows.filter(x => String(x.actorUsername).toLowerCase().includes(q) || String(x.action).toLowerCase().includes(q) || String(x.targetId || "").toLowerCase().includes(q) );
  res.json({ success: true, logs: rows.slice(0, 1000) });
});
app.get("/api/transactions", auth, (req, res) => {
  let rows; if (req.user.role === ROLE.OWNER) { rows = [...db.transactions]; } else { const visibleUsers = new Set(db.users.filter(u => u.id === req.user.id || (u.parentId === req.user.id && u.role === ROLE.RESELLER)).map(u => u.id)); rows = db.transactions.filter(t => visibleUsers.has(t.actorId) || visibleUsers.has(t.targetUserId)); }
  res.json({ success: true, transactions: rows.reverse().slice(0, 1000) });
});
...
app.post("/api/profile/update", auth, (req, res) => {
  const username = normalizeUsername(req.body.username); if (!validUsername(username)) return res.status(400).json({ success: false, message: "Invalid." });
  if (username !== req.user.username) {
    if (Number(req.user.usernameChangeCount || 0) >= 1) return res.status(429).json({ success: false, message: "Limit reached." });
    if (findUserByUsername(username)) return res.status(409).json({ success: false, message: "Exists." });
    req.user.username = username; req.user.usernameChangeCount = Number(req.user.usernameChangeCount || 0) + 1; audit(req.user, "USERNAME_CHANGED", "user", req.user.id, { ip: clientIp(req), details: { username } }); save();
  }
...
app.post("/api/settings/update", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const allowed = [ "panelName", "panelVersion", "supportLink", "sessionTimeoutHours", "resetLimit", "resetWindowHours", "ipGeoLockEnabled", "systemOnline", "onlineMessage", "appNotice", "appNoticeType", "appNoticeEnabled" ];
  for (const field of allowed) {
    if (req.body[field] === undefined) continue;
    if (["sessionTimeoutHours", "resetLimit", "resetWindowHours"].includes(field)) {
      const n = Number(req.body[field]); if (!Number.isFinite(n) || n <= 0) return res.status(400).json({ success: false, message: `Invalid.` });
      if (field === "sessionTimeoutHours" && ![24, 168].includes(Math.floor(n))) return res.status(400).json({ success: false, message: "Invalid timeout." });
      db.settings[field] = Math.floor(n);
    } else if (["ipGeoLockEnabled", "systemOnline", "appNoticeEnabled"].includes(field)) { db.settings[field] = Boolean(req.body[field]); } else { db.settings[field] = sanitizeString(req.body[field], field === "supportLink" ? 500 : 150); }
  }
  audit(req.user, "UPDATE_SETTINGS", "settings", "global", { ip: clientIp(req), details: { fields: allowed.filter(x => req.body[x] !== undefined) } }); save(); res.json({ success: true, settings: db.settings });
});
app.post("/api/settings/loader", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const b = req.body || {};
  if (b.enabled !== undefined) db.settings.loaderStatus = Boolean(b.enabled);
  if (b.message !== undefined) db.settings.maintenanceMsg = sanitizeString(b.message, 1000);
  if (b.loaderStatus !== undefined) db.settings.loaderStatus = Boolean(b.loaderStatus);
  if (b.maintenanceMsg !== undefined) db.settings.maintenanceMsg = sanitizeString(b.maintenanceMsg, 1000);
  if (b.loaderVersion !== undefined) db.settings.loaderVersion = normalizeVersion(b.loaderVersion);
  if (b.loaderMinimumVersion !== undefined) db.settings.loaderMinimumVersion = normalizeVersion(b.loaderMinimumVersion);
  ...
  if (b.loaderUpdateRequired !== undefined) db.settings.loaderUpdateRequired = Boolean(b.loaderUpdateRequired);
  if (b.loaderUpdateNotes !== undefined) db.settings.loaderUpdateNotes = sanitizeString(b.loaderUpdateNotes, 4000);
  audit(req.user, "UPDATE_LOADER_SETTINGS", "settings", "global", { ip: clientIp(req), details: { requestId: req.requestId, loaderStatus: db.settings.loaderStatus } });
  save();
  res.json({ success: true, requestId: req.requestId, settings: loaderPublicConfig(), version: loaderVersionPayload() });
});
app.get("/api/pricing", auth, requireRoles(ROLE.OWNER, ROLE.ADMIN, ROLE.RESELLER), (req, res) => { res.json({ success: true, pricing: db.pricing }); });
app.post("/api/pricing/update", auth, requireRoles(ROLE.OWNER), (req, res) => {
  const incoming = req.body.pricing; if (!incoming || typeof incoming !== "object" || Array.isArray(incoming)) return res.status(400).json({ success: false, message: "Invalid." });
  const next = {};
  for (const key of Object.keys(PRICING)) {
    const row = incoming[key]; if (!row) return res.status(400).json({ success: false, message: `Missing ${key}.` });
    const hours = Number(row.hours); const price = Number(row.price); const label = sanitizeString(row.label || key, 60);
    if (!Number.isFinite(hours) || hours <= 0 || !Number.isFinite(price) || price < 0) return res.status(400).json({ success: false, message: `Invalid.` });
    next[key] = { label, hours, price: Number(price.toFixed(2)) };
  }
  db.pricing = next; audit(req.user, "UPDATE_PRICING", "pricing", "global", { ip: clientIp(req), details: next }); save(); res.json({ success: true, pricing: db.pricing });
});
app.post("/api/owner/enter-user/:id", auth, requireRoles(ROLE.OWNER), (req, res) => {
  ...
  if (target.status !== STATUS.ACTIVE) return res.status(403).json({ success: false, message: "Blocked." });
  const token = createSession(target, req, false, true); audit(req.user, "OWNER_ENTER_USER_PANEL", "user", target.id, { ip: clientIp(req), details: { supportSession: true } }); save();
...
:root{
  ...
  --panel2:#101a29;
  --panel3:#142235;
  --line:rgba(45,214,255,.18);
  --line2:rgba(255,166,0,.18);
  --text:#edf7ff;
  --muted:#71859e;
  --accent:#24d9ff;
  --accent2:#8b5cff;
  ...
  --red:#ff5571;
  --sidebar:260px;
  --glow:0 0 18px rgba(36,217,255,.18),0 0 45px rgba(139,92,255,.08);
...
html,body{
  ...
  background:
    radial-gradient(circle at 15% 10%,rgba(36,217,255,.07),transparent 28%),
    radial-gradient(circle at 85% 18%,rgba(139,92,255,.07),transparent 30%),
    linear-gradient(135deg,#04060b 0%,#070b13 52%,#05070d 100%);
  color:var(--text);
  font:15px 'Rajdhani',system-ui,sans-serif;
  letter-spacing:.15px;
}
body:before{
...
button{cursor:pointer;transition:all .22s ease}
.hidden{display:none!important}
#app{min-height:100vh;position:relative;z-index:1}
.brand{
  font:700 22px 'Orbitron',sans-serif;
  letter-spacing:1px;
  text-shadow:0 0 12px rgba(36,217,255,.55),0 0 28px rgba(139,92,255,.2);
}
.brand span{
  background:linear-gradient(90deg,#43e8ff,#9d75ff 52%,#ffc24b);
...
.sidebar{
  ...
  background:linear-gradient(180deg,rgba(8,14,24,.97),rgba(5,9,16,.98));
  backdrop-filter:blur(18px);
  border-right:1px solid rgba(36,217,255,.16);
  padding:22px 15px;z-index:50;display:flex;flex-direction:column;
  box-shadow:14px 0 55px rgba(0,0,0,.55),inset -1px 0 rgba(139,92,255,.08);
}
.sidebar:after{
  content:"";position:absolute;right:-1px;top:9%;width:2px;height:34%;
  background:linear-gradient(180deg,transparent,var(--accent),var(--accent2),transparent);
  box-shadow:0 0 14px var(--accent);
}
.side-user{
  padding:17px;margin:18px 0 16px;
  background:linear-gradient(135deg,rgba(36,217,255,.08),rgba(139,92,255,.08));
  border:1px solid rgba(36,217,255,.17);border-radius:15px;
  box-shadow:inset 0 1px rgba(255,255,255,.06),var(--glow);
}
.side-nav{display:grid;gap:7px}
.navbtn{
  width:100%;text-align:left;background:transparent;color:#8295ad;
  border:1px solid transparent;border-radius:12px;padding:12px 14px;
  font-weight:600;display:flex;align-items:center;gap:10px;
  position:relative;overflow:hidden;
}
.nav-icon{display:inline-flex;width:26px;min-width:26px;justify-content:center;align-items:center;font-size:17px;filter:drop-shadow(0 0 7px rgba(0,229,255,.22));transition:transform .2s ease}.navbtn:hover .nav-icon,.navbtn.active .nav-icon{transform:scale(1.08)}
.navbtn:before{
  content:"";position:absolute;left:0;top:18%;bottom:18%;width:2px;
  background:linear-gradient(var(--accent),var(--accent2));
  transform:scaleY(0);transition:.2s;box-shadow:0 0 10px var(--accent);
}
.navbtn:hover,.navbtn.active{
  background:linear-gradient(90deg,rgba(36,217,255,.12),rgba(139,92,255,.07),transparent);
  color:#e9fbff;border-color:rgba(36,217,255,.2);
  box-shadow:inset 0 0 20px rgba(36,217,255,.04),0 0 16px rgba(36,217,255,.08);
}
.navbtn:hover:before,.navbtn.active:before{transform:scaleY(1)}
.side-bottom{margin-top:auto;display:grid;gap:8px}

.main{margin-left:var(--sidebar);min-height:100vh}
.topbar{
  height:76px;padding:0 28px;border-bottom:1px solid rgba(36,217,255,.14);
  background:rgba(6,10,18,.82);backdrop-filter:blur(18px);
...
.topbar:after{
  ...
  background:linear-gradient(90deg,transparent,#24d9ff,transparent,#8b5cff,transparent);
  opacity:.65;box-shadow:0 0 12px rgba(36,217,255,.45);
}
.side-overlay{display:none}
.mobile-menu{display:none}
.layout{max-width:1450px;margin:auto;padding:30px}
.page-head{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:24px}
.page-head h1{
  margin:0 0 6px;font:700 30px 'Orbitron',sans-serif;letter-spacing:.4px;
  text-shadow:0 0 18px rgba(36,217,255,.22);
}
.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:17px}
.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:17px}

.card{
  background:linear-gradient(145deg,rgba(14,23,37,.96),rgba(8,13,22,.96));
  border:1px solid rgba(36,217,255,.15);border-radius:17px;padding:22px;
...
.card:after{
  ...
  background:linear-gradient(115deg,rgba(36,217,255,.055),transparent 35%,rgba(139,92,255,.035) 70%,transparent);
}
.card:before{
  content:"";position:absolute;top:0;left:8%;right:8%;height:2px;
  background:linear-gradient(90deg,transparent,#24d9ff 30%,#8b5cff 68%,#ffb52e,transparent);
  box-shadow:0 0 12px rgba(36,217,255,.65),0 0 24px rgba(139,92,255,.25);
  pointer-events:none;
}
.card:hover{
  transform:translateY(-2px);border-color:rgba(36,217,255,.38);
  box-shadow:0 20px 55px rgba(0,0,0,.68),0 0 28px rgba(36,217,255,.13),0 0 45px rgba(139,92,255,.07);
}
.stat .label{color:#7890aa;font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:1.4px}
.stat b{
  display:block;font-size:32px;margin-top:10px;font-weight:800;color:#f4fbff;
  text-shadow:0 0 14px rgba(36,217,255,.28);
}
...
.table th,.table td{padding:14px;border-bottom:1px solid rgba(36,217,255,.09);text-align:left;vertical-align:middle}
.table th{
  color:#7089a5;font-weight:700;text-transform:uppercase;font-size:11px;
  letter-spacing:1.1px;background:rgba(36,217,255,.025);
}
.table tbody tr{transition:.2s}
.table tbody tr:hover{
  background:linear-gradient(90deg,rgba(36,217,255,.07),rgba(139,92,255,.035),transparent);
}
.badge{
  display:inline-block;padding:5px 12px;border-radius:99px;background:rgba(16,26,41,.9);
  font-size:12px;font-weight:700;border:1px solid rgba(36,217,255,.16);
}
.badge.ok{color:var(--green);background:rgba(53,227,138,.08);border-color:rgba(53,227,138,.3);box-shadow:0 0 12px rgba(53,227,138,.16)}
.badge.bad{color:var(--red);background:rgba(255,85,113,.08);border-color:rgba(255,85,113,.3);box-shadow:0 0 12px rgba(255,85,113,.16)}
...
.primary{
  background:linear-gradient(110deg,#16cfe9,#347dff 58%,#7d4cff);color:#fff;
  box-shadow:0 5px 20px rgba(36,217,255,.22),0 0 18px rgba(139,92,255,.12);
}
.primary:hover{transform:translateY(-1px);filter:brightness(1.12);box-shadow:0 8px 28px rgba(36,217,255,.3),0 0 24px rgba(139,92,255,.2)}
.action{background:rgba(16,26,41,.9);color:#c9d9e9;border-color:rgba(36,217,255,.16)}
.action:hover{background:rgba(36,217,255,.1);color:#fff;border-color:rgba(36,217,255,.42);box-shadow:0 0 14px rgba(36,217,255,.12)}
.danger{background:rgba(255,85,113,.07);color:#ff6d86;border-color:rgba(255,85,113,.26)}
.danger:hover{background:rgba(255,85,113,.13);box-shadow:0 0 16px rgba(255,85,113,.18)}
.full{width:100%}

.field{margin:16px 0}
.field label{
  display:block;margin-bottom:8px;color:#86a0bb;font-weight:700;
  font-size:12px;letter-spacing:1px;text-transform:uppercase;
}
input,select,textarea{
  width:100%;background:linear-gradient(135deg,rgba(5,9,16,.96),rgba(12,19,31,.96));
  color:var(--text);border:1px solid rgba(36,217,255,.14);border-radius:11px;
  padding:13px 14px;outline:none;transition:all .22s;
  box-shadow:inset 0 0 14px rgba(0,0,0,.18);
}
input::placeholder,textarea::placeholder{color:#42556d}
input:hover,select:hover,textarea:hover{border-color:rgba(36,217,255,.25)}
input:focus,select:focus,textarea:focus{
  border-color:var(--accent);box-shadow:0 0 0 2px rgba(36,217,255,.06),0 0 20px rgba(36,217,255,.18);
}
...
.key{
  ...
  background:rgba(36,217,255,.055);padding:4px 8px;border-radius:6px;color:#63e8ff;
  border:1px solid rgba(36,217,255,.2);text-shadow:0 0 8px rgba(36,217,255,.25);
}

.login-wrap{
  min-height:100vh;display:grid;place-items:center;padding:20px;
  background:
    radial-gradient(circle at 50% 15%,rgba(36,217,255,.1),transparent 25%),
    radial-gradient(circle at 70% 75%,rgba(139,92,255,.08),transparent 30%),
    linear-gradient(135deg,#04060b,#080d17 55%,#05070d);
  position:relative;
}
.login-wrap:before{
  content:"";position:absolute;width:520px;height:520px;border:1px solid rgba(36,217,255,.08);
  border-radius:50%;box-shadow:0 0 80px rgba(36,217,255,.04),inset 0 0 80px rgba(139,92,255,.03);
}
.login{
  width:min(440px,100%);position:relative;z-index:1;
  background:linear-gradient(145deg,rgba(12,20,32,.94),rgba(6,11,19,.96));
  backdrop-filter:blur(22px);border:1px solid rgba(36,217,255,.23);
  border-radius:20px;padding:36px;
  box-shadow:0 30px 80px rgba(0,0,0,.78),0 0 35px rgba(36,217,255,.1),0 0 65px rgba(139,92,255,.05);
}
.login:before{
  content:"";position:absolute;top:-1px;left:10%;right:10%;height:2px;
  background:linear-gradient(90deg,transparent,#24d9ff,#8b5cff,#ffb52e,transparent);
  box-shadow:0 0 16px rgba(36,217,255,.65);
}
.error{color:#ff7188;background:rgba(255,85,113,.08);border:1px solid rgba(255,85,113,.3);padding:12px;border-radius:10px;margin:12px 0;font-weight:600}
.flash{margin-bottom:16px;padding:14px 18px;border-radius:11px;font-weight:700;border:1px solid;display:flex;align-items:center;gap:10px}
.flash.success{color:var(--green);background:rgba(53,227,138,.07);border-color:rgba(53,227,138,.3);box-shadow:0 0 16px rgba(53,227,138,.08)}
.flash.error{color:var(--red);background:rgba(255,85,113,.07);border-color:rgba(255,85,113,.3);box-shadow:0 0 16px rgba(255,85,113,.08)}
.toast{
  position:fixed;right:24px;bottom:24px;background:linear-gradient(135deg,#0e1a29,#111d30);
  border:1px solid rgba(36,217,255,.55);color:#fff;padding:14px 20px;border-radius:13px;z-index:100;
  box-shadow:0 15px 35px rgba(0,0,0,.7),0 0 22px rgba(36,217,255,.18);font-weight:700;
}
.modal-bg{
  position:fixed;inset:0;background:rgba(1,4,9,.84);backdrop-filter:blur(10px);
  display:grid;place-items:center;padding:20px;z-index:90;
}
.modal{
  width:min(650px,100%);max-height:90vh;overflow:auto;
  background:linear-gradient(145deg,rgba(13,23,37,.98),rgba(6,11,19,.99));
  border:1px solid rgba(36,217,255,.32);border-radius:20px;padding:28px;
  box-shadow:0 30px 80px rgba(0,0,0,.86),0 0 38px rgba(36,217,255,.1),0 0 55px rgba(139,92,255,.06);
  position:relative;
}
.modal:before{
  content:"";position:absolute;top:0;left:8%;right:8%;height:2px;
  background:linear-gradient(90deg,transparent,#24d9ff,#8b5cff,#ffb52e,transparent);
  box-shadow:0 0 15px rgba(36,217,255,.5);
}
.modal h2{margin-top:0;font:700 23px 'Orbitron',sans-serif;margin-bottom:16px;color:#6feaff;text-shadow:0 0 12px rgba(36,217,255,.28)}
.password-wrap{position:relative}
.password-wrap input{padding-right:48px}
.eye-btn{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:36px;height:36px;border:0;background:transparent;color:#60748c;padding:0;display:grid;place-items:center}
.eye-btn:hover{color:#fff;text-shadow:0 0 10px var(--accent)}
.eye-btn svg{width:20px;height:20px;stroke:currentColor;fill:none;stroke-width:2}
.setting-row{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:16px 0;border-bottom:1px solid rgba(36,217,255,.09)}
.switch{position:relative;width:52px;height:28px}
.switch input{display:none}
.switch span{position:absolute;inset:0;background:#172337;border-radius:99px;transition:.3s;border:1px solid rgba(36,217,255,.15)}
.switch span:after{content:"";position:absolute;width:20px;height:20px;left:3px;top:3px;background:#dcecff;border-radius:50%;transition:.3s;box-shadow:0 2px 8px rgba(0,0,0,.4)}
.switch input:checked+span{background:linear-gradient(90deg,#16cfae,#238cff);border-color:#35dfff;box-shadow:0 0 16px rgba(36,217,255,.28)}
.switch input:checked+span:after{transform:translateX(24px)}

@media(max-width:900px){
  :root{--sidebar:0px}
  .sidebar{display:none}
  .sidebar.open{display:flex;width:280px;box-shadow:25px 0 70px rgba(0,0,0,.88)}
  .side-overlay{position:fixed;inset:0;background:rgba(0,0,0,.72);backdrop-filter:blur(4px);z-index:40}
  .side-overlay.open{display:block}
  .main{margin-left:0}
  .mobile-menu{display:inline-flex;padding:8px}
...
function keySuccessCard(keys,title='Key Generated Successfully'){if(!keys||!keys.length)return '';return '<div class="card" style="margin-bottom:14px;border-color:var(--ok)"><h3 style="margin:0 0 10px;color:var(--ok)">'+esc(title)+'</h3>'+keys.map(k=>'<div style="padding:10px 0;border-bottom:1px solid var(--line)"><div class="row" style="justify-content:space-between;gap:8px"><b class="key">'+esc(k.key)+'</b><button class="action" onclick="copyText(\''+esc(k.key)+'\')">Copy</button></div><small>Duration: '+esc(k.durationLabel||k.duration||'')+' · Devices: '+Number(k.deviceLimit||1)+' · Status: '+esc(k.status||'active')+' · Expiry: '+esc(k.expiryAt?fmt(k.expiryAt):'Not Started')+'</small></div>').join('')+'</div>'}
...
function navItem(id,label){const icons={dashboard:'🏠',generateKeys:'⚡',manageKeys:'🔑',keyManage:'🛠️',users:'👥',referrals:'🎯',transactions:'💳',activity:'📊',audit:'🛡️',settings:'⚙️',loader:'🎮'};return '<button data-page="'+esc(id)+'" class="navbtn '+(currentPage===id?'active':'')+'" onclick="page(\''+id+'\')"><span class="nav-icon" aria-hidden="true">'+(icons[id]||'◈')+'</span><span>'+esc(label)+'</span></button>'}
const PAGE_PATHS={dashboard:'/Dashboard',generateKeys:'/Generate-Keys',manageKeys:'/Manage-Keys',keyManage:'/Manage-Key',users:'/Users',referrals:'/Referrals',transactions:'/Transactions',activity:'/Activity',audit:'/Audit-Logs',settings:'/Settings',loader:'/Loader'};
...
function sidebar(){let owner=currentUser.role==='owner',admin=currentUser.role==='admin';let items=navItem('dashboard','Dashboard')+navItem('generateKeys','Generate Keys')+navItem('manageKeys','Manage Keys');if(owner)items+=navItem('loader','Loader');if(owner)items+=navItem('users','Users');if(owner||admin)items+=navItem('referrals','Referrals');if(owner||admin)items+=navItem('transactions','Transactions')+navItem('activity','Activity');if(owner)items+=navItem('audit','Audit Logs')+navItem('settings','Settings');else items+=navItem('settings','Settings');return '<div id="sideOverlay" class="side-overlay" onclick="closeSideMenu()"></div><aside id="sidebar" class="sidebar" onclick="event.stopPropagation()"><div class="brand">Lexi <span>Loader</span></div><div class="side-user"><b>'+esc(currentUser.username)+'</b><div class="muted" style="margin-top:5px">'+esc(currentUser.role.toUpperCase())+'</div><div style="margin-top:7px">Balance: ₹'+Number(currentUser.balance||0).toFixed(2)+'</div></div><div class="side-nav">'+items+'</div><div class="side-bottom">'+(ownerToken&&owner?'<button class="primary full" onclick="returnOwner()">↩️ Return to Owner</button>':'')+'<button class="action full" onclick="logout()">🚪 Logout</button></div></aside>'}
...
async function dashboard(){try{const d=await api('/api/dashboard');const s=d.stats||{};document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Dashboard</h1><div class="muted">License and account overview</div></div><button class="primary" onclick="generateModal()">Generate Keys</button></div><div class="grid">'+stat('Active Keys',s.activeKeys||0)+stat('Expired Keys',s.expiredKeys||0)+stat('Blocked Keys',s.blockedKeys||0)+stat(currentUser.role==='owner'?'Users':'My Keys',s.users||s.activeKeys||0)+'</div><div class="grid2" style="margin-top:16px">'+stat('Balance','₹'+Number(s.balance||0).toFixed(2))+stat('Resellers',s.resellers||0)+'</div><div class="card" style="margin-top:16px"><h3>Loader Status</h3><p><span class="badge '+(d.settings?.loaderStatus?'ok':'bad')+'">'+(d.settings?.loaderStatus?'ONLINE':'MAINTENANCE')+'</span></p><p class="muted">'+esc(d.settings?.loaderStatus?'Loader verification is enabled.':(d.settings?.maintenanceMsg||'Loader disabled.'))+'</p></div><div class="card" style="margin-top:16px"><h3>Account Creation</h3><p class="muted">Exact account creation date and time</p><b>'+esc(fmt(currentUser.createdAt))+'</b></div>'}catch(e){toast(e.message)}}
...
async function editReferralBalance(referralId){try{const d=await api('/api/referrals/'+encodeURIComponent(referralId));const r=d.referral||d;const ures=await api('/api/users/'+encodeURIComponent(r.creatorId));const u=ures.user;if(!u)return toast('Referral user not found.');modal('<h2>Edit Balance</h2><div class="field"><label>Username</label><div class="bracket">'+esc(u.username||r.creatorId)+'</div></div><div class="field"><label>Current Balance</label><div class="bracket">₹'+Number(u.balance||0).toFixed(2)+'</div></div><div class="field"><label>Amount</label><input id="refBalAmount" type="number" min="0" step="0.01" value="0"></div><div class="field"><label>Action</label><select id="refBalMode"><option value="add">Add</option><option value="subtract">Subtract</option><option value="set">Set</option></select></div><div id="refBalErr" class="error hidden"></div><div class="row" style="justify-content:flex-end;gap:8px"><button class="primary" onclick="saveReferralBalance(&quot;'+esc(r.id)+'&quot;)">Save</button><button class="action" onclick="closeModal()">Cancel</button></div>');}catch(e){toast(e.message)}}
async function saveReferralBalance(referralId){const er=document.getElementById('refBalErr');try{const amount=Number(document.getElementById('refBalAmount')?.value);const mode=String(document.getElementById('refBalMode')?.value||'set');if(!Number.isFinite(amount)||amount<0){er.textContent='Invalid balance amount.';er.classList.remove('hidden');return;}await api('/api/referrals/'+encodeURIComponent(referralId)+'/balance',{method:'POST',body:JSON.stringify({mode,amount})});closeModal();setFlash('Balance updated successfully.','success');referralsPage();}catch(e){er.textContent=e.message;er.classList.remove('hidden');}}
...
async function loaderPage(){try{const [c,v,m]=await Promise.all([api('/api/loader/config'),api('/api/loader/version'),api('/api/loader/manifest')]);const s=c.loader||{},ver=v.version||{},man=m.manifests||[];document.getElementById('content').innerHTML='<div class="page-head"><div><h1>Loader</h1><div class="muted">LEXI Loader backend control and update policy</div></div><button class="primary" onclick="loaderPage()">Refresh</button></div><div class="grid2"><div class="card"><h3>Loader Status</h3><div class="setting-row"><div><b>Enabled</b><small>Controls Verification #1 and #2</small></div><label class="switch"><input id="ldEnabled" type="checkbox" '+(s.enabled?'checked':'')+'><span></span></label></div><div class="field"><label>Maintenance Message</label><textarea id="ldMsg" rows="3">'+esc(s.maintenanceMessage||'')+'</textarea></div><button class="primary" onclick="saveLoaderAdmin()">Save Status</button></div><div class="card"><h3>Target Application</h3><div class="field"><label>Authorized Package</label><input id="ldPackage" value="'+esc(s.targetPackage||'')+'" placeholder="com.pubg.imobile"></div><div class="field"><label>Loader API Version</label><input value="'+esc(ver.apiVersion||1)+'" readonly></div><button class="primary" onclick="saveLoaderAdmin()">Save Target</button></div></div><div class="settings-grid" style="margin-top:16px"><div class="card"><h3>Version Policy</h3><div class="field"><label>Current Loader Version</label><input id="ldVersion" value="'+esc(ver.currentVersion||'1.0.0')+'"></div><div class="field"><label>Minimum Supported Version</label><input id="ldMinVersion" value="'+esc(ver.minimumVersion||'1.0.0')+'"></div><div class="setting-row"><div><b>Force Update</b><small>Block launch until update completes</small></div><label class="switch"><input id="ldUpdateRequired" type="checkbox" '+(ver.updateRequired?'checked':'')+'><span></span></label></div></div><div class="card"><h3>Update Package</h3><div class="field"><label>Update URL</label><input id="ldUpdateUrl" value="'+esc(ver.updateUrl||'')+'" placeholder="HTTPS update URL"></div><div class="field"><label>SHA-256</label><input id="ldSha" value="'+esc(ver.updateSha256||'')+'" placeholder="64-character SHA-256"></div><div class="field"><label>Package Size (bytes)</label><input id="ldSize" type="number" min="0" value="'+Number(ver.updateSize||0)+'"></div><div class="field"><label>Release Notes</label><textarea id="ldNotes" rows="3">'+esc(ver.updateNotes||'')+'</textarea></div><button class="primary" onclick="saveLoaderAdmin()">Save Version / Update</button></div></div><div class="card" style="margin-top:16px"><h3>Controlled/Test Feature Entitlements</h3><div class="setting-row"><span>BT</span><label class="switch"><input id="ldBT" type="checkbox" '+(s.features?.BT?'checked':'')+'><span></span></label></div><div class="setting-row"><span>AIM</span><label class="switch"><input id="ldAIM" type="checkbox" '+(s.features?.AIM?'checked':'')+'><span></span></label></div><div class="setting-row"><span>ESP</span><label class="switch"><input id="ldESP" type="checkbox" '+(s.features?.ESP?'checked':'')+'><span></span></label></div><div class="setting-row"><span>IPAD_VIEW</span><label class="switch"><input id="ldIPAD" type="checkbox" '+(s.features?.IPAD_VIEW?'checked':'')+'><span></span></label></div><p class="muted">These values are authorization/configuration flags only; the backend does not modify protected applications.</p><button class="primary" onclick="saveLoaderAdmin()">Save Entitlements</button></div><div class="card" style="margin-top:16px"><h3>File Manifest</h3><p class="muted">Server-side metadata for approved Loader-owned files. Actual copying/integrity verification remains on the Loader.</p><div class="table-wrap"><table class="table"><thead><tr><th>ID</th><th>Version</th><th>Size</th><th>SHA-256</th><th>Required</th><th>Class</th></tr></thead><tbody>'+(man.length?man.map(x=>'<tr><td>'+esc(x.id)+'</td><td>'+esc(x.version)+'</td><td>'+Number(x.size||0)+'</td><td class="key">'+esc(x.sha256||'')+'</td><td>'+((x.required)?'Yes':'No')+'</td><td>'+esc(x.pathClass||'')+'</td></tr>').join(''):'<tr><td colspan="6" class="empty">No file manifests configured.</td></tr>')+'</tbody></table></div></div>'}catch(e){setFlash('Loader page failed: '+e.message,'error');renderFlashOnly()}}
async function saveLoaderAdmin(){try{const sha=String(document.getElementById('ldSha')?.value||'').trim().toLowerCase();if(sha&&!/^[a-f0-9]{64}$/.test(sha))return toast('SHA-256 must be exactly 64 hexadecimal characters.');const payload={loaderStatus:document.getElementById('ldEnabled')?.checked,maintenanceMsg:document.getElementById('ldMsg')?.value,loaderTargetPackage:document.getElementById('ldPackage')?.value.trim(),loaderVersion:document.getElementById('ldVersion')?.value.trim(),loaderMinimumVersion:document.getElementById('ldMinVersion')?.value.trim(),loaderUpdateRequired:document.getElementById('ldUpdateRequired')?.checked,loaderUpdateUrl:document.getElementById('ldUpdateUrl')?.value.trim(),loaderUpdateSha256:sha,loaderUpdateSize:Number(document.getElementById('ldSize')?.value||0),loaderUpdateNotes:document.getElementById('ldNotes')?.value,loaderFeatures:{BT:document.getElementById('ldBT')?.checked,AIM:document.getElementById('ldAIM')?.checked,ESP:document.getElementById('ldESP')?.checked,IPAD_VIEW:document.getElementById('ldIPAD')?.checked}};const d=await api('/api/owner/loader-settings',{method:'POST',body:JSON.stringify(payload)});toast(d.message||'Loader settings saved');await loaderPage()}catch(e){toast(e.message)}}
...
app.get(/^\/(Dashboard|Generate-Keys|Manage-Keys|Manage-Key|Users|Referrals|Transactions|Activity|Audit-Logs|Settings|Online|Loader)(?:\/.*)?$/i, (req, res) => res.type("html").send(PANEL_HTML));
...
setInterval(() => {
  try {
    ...
    cleanRateLimits();
    if (before !== db.loaderSessions.length) save();
  } catch (err) { console.error("[Lexi Loader] maintenance sweep failed:", err.message); }
}, 5 * 60 * 1000).unref?.();
